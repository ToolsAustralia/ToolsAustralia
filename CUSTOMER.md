...
| `subscription.pausedFrom` / `pausedUntil` | Date (opt) | **Retention-pause window** — set when a member accepts the 30-day `pause_30d` offer (§5.6). `pausedFrom` = the member's period end (freeze begins); `pausedUntil` = auto-resume date (`pausedFrom + 30d`). While `pausedFrom ≤ now < pausedUntil` the member is frozen (`status="paused"`, `isActive=false`); cleared on resume ([User.ts:50-51](src/models/User.ts#L50)). See [BUSINESS.md §10a](BUSINESS.md) | — |
