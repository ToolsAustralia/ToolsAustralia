/**
 * Convert PNG multiplier banners under public/images/banners to WebP, then remove PNGs.
 * Run: npx tsx scripts/convert-multiplier-banners-to-webp.ts
 */
...
const BANNERS_ROOT = path.join(process.cwd(), "public", "images", "banners");
...
async function main() {
  const pngs = await collectPngFiles(BANNERS_ROOT);
  if (pngs.length === 0) {
    console.log("No PNG files under", BANNERS_ROOT);
    return;
  }
