/**
 * Convert PNG assets under public/images/background/promo/landing to WebP, then remove PNGs.
 * Run: npx tsx scripts/convert-promo-landing-to-webp.ts
 */
...
async function main() {
  const pngs = await collectPngFiles(LANDING_ROOT);
  if (pngs.length === 0) {
    console.log("No PNG files under", LANDING_ROOT);
    return;
  }
