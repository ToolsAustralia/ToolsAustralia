...
| `/api/cron/cancellation-retention-resume` | `0 16 * * *` | 300s | Clears stale `pauseReason="retention"` metadata on Stripe subscriptions after the 30-day retention pause window has elapsed. See [architecture.md](./architecture.md#vercel-cron-schedules). |
...
`GET /api/cron/sync-tiktok-ads` — `src/app/api/cron/sync-tiktok-ads/route.ts`. Scheduled `45 2 * * *` (`maxDuration: 300s`). The TikTok analogue of `sync-meta-ads`: re-pulls a trailing 8-day window (`since = now - 7d`, `until = now`, `Australia/Sydney`-formatted) of TikTok ad-level insights and upserts them into `TikTokAdInsightsDaily` via `new TikTokInsightsSyncService().syncDateRange(...)`. The wide re-pull captures TikTok's later revisions to recent days. Auth: `Authorization: Bearer ${CRON_SECRET}` (skipped when `CRON_SECRET` is unset, local dev). Returns `200 { ok: false, skipped: true, reason: "env" }` when `isTikTokAdInsightsConfigured()` is false (TikTok Marketing-API creds `TIKTOK_ADVERTISER_ID` / `TIKTOK_MARKETING_ACCESS_TOKEN` absent), else `{ ok: true, rowsUpserted, adIds, durationMs }`.
