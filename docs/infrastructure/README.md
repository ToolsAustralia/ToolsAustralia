...
- **npm scripts:** `build:chat-knowledge-pack` (regenerates `src/generated/chatKnowledgePack.ts` from canonical data — chained into `prebuild`/`predev`); tests `test:chat-faqs`, `test:chat-models`, `test:chat-cost-guard`, `test:chat-provider`, `test:chat-knowledge`, `test:chat-deflection` (no-LLM deflection layer — offline, no API key required), `test:chat-escalation` (stubbed escalation + system-prompt assertions — no Mongo/SendGrid required); `smoke:chat-provider` (one live Anthropic call — manual connectivity check, not CI).
