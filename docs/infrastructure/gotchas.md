...
## `membership-daily-snapshot`'s second daily fire silently overwrote the first (2026-08-24)
...
**Why this mattered specifically on renewal nights.** ~900 memberships renew right at the day boundary (00:00 AEST = 14:00 UTC). The first snapshot fire (originally 14:00 UTC, moved to 18:00 UTC — see below) can land before that burst's webhooks have all been processed, so it captures the TRUE end-of-day figure for the day that just closed. The second fire (originally 15:00 UTC, moved to 19:00 UTC), an hour later, lands after most of the burst has been processed — its "live" counts now reflect an extra hour of the NEXT day's renewal activity bleeding into what is still labelled as the PREVIOUS day's snapshot. The unconditional overwrite meant the surviving row was always the contaminated post-burst one, on exactly the nights this metric matters most.

**Fix:** `upsertMembershipSnapshotRow` (exported from [`route.ts`](../../src/app/api/cron/membership-daily-snapshot/route.ts)) now checks for an existing row at that `(date, packageId)` key before writing; if one exists, the write is skipped (`written: false`) and the second fire becomes a true no-op fallback rather than a blind overwrite. The two round trips are not atomic, but the two cron fires are scheduled an hour apart (never concurrent) and the model's unique `{date, packageId}` index is a hard backstop. Regression test: `npm run test:membership-snapshot-write-once` ([`membership-daily-snapshot.test.ts`](../../src/app/api/cron/__tests__/membership-daily-snapshot.test.ts)) — seeds a first write, attempts a second with different ("post-burst") counts, and asserts the row is unchanged.

**`dashboard-stats-daily-snapshot` does NOT need this guard** even though it also fires twice into the same day — its writer (`writeSlidingWindow` → `mergeAdChannels`) already does a field-level merge that prefers a successful fetch over the stored value and only preserves the old value when a provider errors (see [gotchas.md](../metrics-analytics/gotchas.md)), so a later re-run corrects a stale day and can never blank a good one. The two crons' "second fire" have opposite intents: `dashboard-stats-daily-snapshot` wants the second fire to WIN (self-healing); `membership-daily-snapshot` wants the FIRST fire to win (uncontaminated by next-day processing). Don't copy one cron's redundancy pattern onto the other without checking which direction is correct.

Both crons were also moved off `0 14`/`0 15 * * *` on 2026-08-24, to `0 18`/`0 19 * * *` — those two UTC hours carried 2,235 and 3,551 Stripe webhook events respectively on the 24 Aug renewal night (payment events trail invoice events by ~1h because Stripe finalizes subscription drafts after creating them), so seven-plus cron jobs competing with that DB load in the same minute was its own incident. `18:00`/`19:00` UTC = `04:00`/`05:00` AEST = `05:00`/`06:00` AEDT — quiet in both DST regimes.
