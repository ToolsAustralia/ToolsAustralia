...
- [`scripts/dump-promo-config.ts`](../../scripts/dump-promo-config.ts) — **read-only** export of the promo multiplier config (`Promo` + `ScheduledPromo`) to a timestamped JSON file under `/temp` (gitignored). npm scripts: `dump:promo-config` (whatever `MONGODB_URI` points at), `dump:promo-config:prod` (`PROD_MONGODB_URI`, db `Production`, via `connectOpsDb`). Loads `.env.local` with `dotenv` **before** importing `connect-ops-db`, because `tsx` does not load it automatically and `connectOpsDb` reads the URIs at call time. Prints per-collection counts up front, a line per exported collection, and a final summary. **Exit codes:** 0 ok / 1 connect or read failure / 2 completed but zero documents (usually means the wrong database).

  First prod run (2026-08-04): 229 `ScheduledPromo` (101 active, all `membership-packages`, multipliers ×2/×3/×5/×10) and **0 `Promo`** — the legacy collection is empty in production.

- [`scripts/seed-promo-config.ts`](../../scripts/seed-promo-config.ts) — loads a dump back in. **Dry-run by default**; `--apply` writes. npm scripts: `seed:promo-config:dry`, `seed:promo-config`. Flags: `--file=<path>` (default: newest dump in `/temp`), `--from=`/`--to=` (default 2026-07-28 → 2026-08-27; **overlap**, not containment, so a promo starting before the window but running into it still counts). **Exit codes:** 0 ok / 1 fatal or guard trip / 2 completed with per-document errors.

  **The trap this script is built around: `MONGODB_URI` in `.env.local` is a remote SHARED Atlas cluster (`tools-australia.…`), not a local database** — and because that URI carries no `/<dbName>` path, Mongo falls back to a database literally named `test`. "Seed my local DB" therefore writes somewhere other people read. Three rails follow from that: `--prod` is refused outright; the run aborts if the resolved host matches `PROD_MONGODB_URI`; and every existing document in the target collections is backed up to `/temp` before the first write (that file is the undo). Writes are `replaceOne(..., { upsert: true })` keyed by `_id` — **nothing is ever deleted**, so documents in the target that are not in the dump survive untouched.
...
|---|---|---|
| `/api/cron/major-draw-transition` | `15 18 * * *` | Daily — advances draw lifecycle. Backstop only (real-time traffic-driven transitions are primary). **Moved off `0 14 * * *` on 2026-08-24** to clear the renewal-webhook-burst hour — see [gotchas.md](./gotchas.md). |
| `/api/cron/process-partner-discount-queues` | `45 18 * * *` | Daily — clears partner discount queue. **Moved off `0 15 * * *` on 2026-08-24** to clear the renewal-webhook-burst hour — see [gotchas.md](./gotchas.md). |
...
| `/api/cron/sync-tiktok-ads` | `0 * * * *` + `59 12,13 * * *` | **Every 3 hours** (Sydney-local 3/6/9/12/15/18/21 + 23:59; over-invoked hourly from Vercel and gated in-handler because Vercel cron is UTC and DST-blind — identical to `sync-meta-ads`). Re-syncs a trailing 8-day window of TikTok ad-level insights into `TikTokAdInsightsDaily` via the shared `runSpendByUrlSync`. **Was `45 2 * * *` (once daily) until 2026-08-11** — an oversight, not a decision: TikTok shipped nightly on 2026-07-16 and kept that cadence when the shared pipeline landed on 2026-07-29, even though Meta had run it 3-hourly all along. The daily cadence left the current day's TikTok spend invisible until the next morning, and every downstream read (Advertising card, blended ROAS, MER, spend-by-URL) inherited it. Bearer `CRON_SECRET` gate; no-ops when the TikTok Marketing-API env is unset. `maxDuration: 300s`. |
| `/api/cron/membership-daily-snapshot` | `30 17 * * *` and `30 20 * * *` | Daily ×2 — writes yesterday's `MembershipDailySnapshot` per package. **Write-once upsert with a degenerate-row escape hatch (2026-08-24)**: the first run to reach a given `(date, packageId)` with a non-degenerate result wins; the second fire is a no-op fallback for a missed/failed first run (or self-heals a degenerate all-zero row) rather than an unconditional overwrite — see [gotchas.md § write-once snapshot guard](./gotchas.md). **The census is NOT point-in-time** — `getMembershipByPackageLiveForSnapshot` has zero date filtering, so accuracy is purely a function of how many hours past the Sydney day boundary the cron fires; the guard keeps whichever run is CLOSER to the boundary, not a "clean" run in any absolute sense. **Moved off `0 14`/`0 15`, then off `0 18`/`0 19`, on 2026-08-24** — see the `dashboard-stats-daily-snapshot` row below for why `0 18`/`0 19` didn't stick. `maxDuration: 300s` (in-file `export const maxDuration`, deliberately not only in `vercel.json` — see gotchas.md). |
| `/api/cron/dashboard-stats-daily-snapshot` | `30 17 * * *`, `30 20 * * *` and `20 3 * * *` | Daily ×3 — re-upserts a 90-day sliding window of `DashboardStatsDailySnapshot` rows. The second fire heals first-run failures (this writer's `mergeAdChannels` already takes a successful fetch over the stored value, so no write-once guard is needed here — see [gotchas.md](./gotchas.md)). **The third MUST stay after `sync-tiktok-ads`**: an AEST day ends at 14:00 UTC but TikTok keeps attributing conversions for hours, so an early snapshot always freezes a partial TikTok figure — measured on 2026-08-10 as $386.82/$40.00 (ROAS 0.103) against an actual $410.93/$90.00 (ROAS 0.219). **Moved off `0 14`/`0 15`, then off `0 18`/`0 19`, on 2026-08-24**: the first move cleared the renewal-webhook-burst hour; the second move fixed a real bug in the first — `0 19` collides with `sync-tiktok-ads`/`sync-meta-ads`'s in-handler Sydney-wall-clock slot gate during AEDT (19:00 UTC = Sydney 06:00 AEDT, one of their `{3,6,9,12,15,18,21}:00` slots), and this cron reads the ad-insights tables those syncs are mid-write on. `30 17`/`30 20` (a `:30` minute) structurally avoids this in both DST regimes — see gotchas.md. Idempotent. `maxDuration: 300s`. |
...
| `/api/cron/reconcile-renewal-grants` | `40 3 * * *` | Daily — the **paid-but-not-granted detector** (owned by [billing-stripe](../billing-stripe/architecture.md#renewal-grant-reconciliation--the-paid-but-not-granted-detector-2026-08-24)). Anti-joins paid `subscription_cycle` cycles against their `BenefitsGranted` PaymentEvent, and reports every `dead` `stripewebhookqueue` row. Read-only: no Mongo write, no Stripe call. **Auth fails CLOSED** — unlike the `if (!cronSecret) return true` siblings, a missing `CRON_SECRET` returns 401. Window is `now−8h` back 48h **on `updatedAt`** (a dunning-recovered row's `createdAt` is pinned to its failure date, which would make a `createdAt` window permanently false-clean for exactly the past-due-recovery population); the 8h settle margin clears the webhook queue's 7h21m retry ladder so an in-flight retry is never reported as a loss. Clean runs emit a `console.error` heartbeat so "ran and clean" is distinguishable from "never fired". **Must not move to 14:00 or 15:00 UTC** — those are the anchor-24 renewal burst and its payment wave. `maxDuration: 60s`. |
...
`Australia/Sydney`-anchored snapshot crons (`membership-daily-snapshot`, `dashboard-stats-daily-snapshot`) fire at 17:30 UTC = 03:30 AEST / 04:30 AEDT, and 20:30 UTC = 06:30 AEST / 07:30 AEDT — both are well after Sydney midnight in either DST regime, so they reliably write "yesterday" in local time. The date-key formula itself is invariant to which fixed UTC hour is chosen (only which side of Sydney midnight / the DST transition instant matters, not the specific hour), which is WHY this schedule has moved twice in the same day without ever risking the formula: 14:00/15:00 UTC → 18:00/19:00 UTC → 17:30/20:30 UTC, all on 2026-08-24. The middle step didn't stick — `19:00` UTC collides with the `sync-meta-ads`/`sync-tiktok-ads` in-handler Sydney-slot gate during AEDT (see [gotchas.md](./gotchas.md)) — but every step still wrote the correct "yesterday" key. See [`docs/subscription/architecture.md`](../subscription/architecture.md) for the full membership-snapshot flow and `scripts/test-membership-snapshot-dst.ts` for the DST verification test.
