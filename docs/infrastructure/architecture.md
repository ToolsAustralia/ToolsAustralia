...
| `/api/cron/dashboard-stats-daily-snapshot` | `0 14 * * *`, `0 15 * * *` and `20 3 * * *` | Daily ×3 — re-upserts a 90-day sliding window of `DashboardStatsDailySnapshot` rows. The second fire heals first-run failures. **The third MUST stay after `sync-tiktok-ads` (02:45 UTC)**: an AEST day ends at 14:00 UTC but TikTok keeps attributing conversions for hours, so the 14:00/15:00 snapshot always freezes a partial TikTok figure — measured on 2026-08-10 as $386.82/$40.00 (ROAS 0.103) against an actual $410.93/$90.00 (ROAS 0.219). Idempotent. `maxDuration: 300s`. |
