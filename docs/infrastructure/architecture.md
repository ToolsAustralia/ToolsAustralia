...
| `/api/cron/membership-daily-snapshot` | `0 18 * * *` and `0 19 * * *` | Daily ×2 — writes yesterday's `MembershipDailySnapshot` per package. **Write-once upsert (2026-08-24)**: the first run to reach a given `(date, packageId)` wins; the second fire is a true no-op fallback for a missed/failed first run, not a redundant overwrite — see [gotchas.md § write-once snapshot guard](./gotchas.md). **Moved off `0 14`/`0 15` on 2026-08-24** — those are the ~900-membership renewal-webhook burst (2,235 events) and its trailing Stripe payment wave (3,551 events, ~1h later, since Stripe finalizes subscription drafts roughly an hour after creating them); a same-day overwrite on a renewal-burst night meant the surviving row was POST-burst, contaminated by an extra hour of next-day renewal processing, not the true end-of-day figure. |
| `/api/cron/dashboard-stats-daily-snapshot` | `0 18 * * *`, `0 19 * * *` and `20 3 * * *` | Daily ×3 — re-upserts a 90-day sliding window of `DashboardStatsDailySnapshot` rows. The second fire heals first-run failures (this writer's `mergeAdChannels` already takes a successful fetch over the stored value, so no write-once guard is needed here — see [gotchas.md](./gotchas.md)). **The third MUST stay after `sync-tiktok-ads`**: an AEST day ends at 14:00 UTC but TikTok keeps attributing conversions for hours, so an early snapshot always freezes a partial TikTok figure — measured on 2026-08-10 as $386.82/$40.00 (ROAS 0.103) against an actual $410.93/$90.00 (ROAS 0.219). **Moved off `0 14`/`0 15` on 2026-08-24** to clear the renewal-surge hour (same reason as `membership-daily-snapshot` above). Idempotent. `maxDuration: 300s`. |
...
`Australia/Sydney`-anchored snapshot crons (`membership-daily-snapshot`, `dashboard-stats-daily-snapshot`) fire at 18:00 UTC = 04:00 AEST / 05:00 AEDT, and 19:00 UTC = 05:00 AEST / 06:00 AEDT — both are well after Sydney midnight in either DST regime, so they reliably write "yesterday" in local time. (They fired at 14:00/15:00 UTC — 00:00/01:00 AEST, 01:00/02:00 AEDT — until 2026-08-24; the date-key formula is invariant to which fixed UTC hour is chosen as long as it doesn't straddle Sydney midnight or a DST transition instant, so the move only changed which quiet hour the write lands in, not correctness.) See [`docs/subscription/architecture.md`](../subscription/architecture.md) for the full membership-snapshot flow and `scripts/test-membership-snapshot-dst.ts` for the DST verification test.
