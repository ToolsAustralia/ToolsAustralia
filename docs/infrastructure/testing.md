...
npm run test:ack-gate                # the webhook-queue ACK gate (incident 2026-08-23, 11 members charged $300.00 with no entries): a handler that returns normally is NOT automatically a success. Asserts an ungranted invoice.payment_succeeded leaves the row retryable and writes NO ProcessedStripeEvent row (that unique row is what blocked replay-based healing), that ordinary non-payment events (shouldMarkAsProcessed:false) still mark succeeded, that a real Stripe 429 inside the handler requeues with the true error text in lastError, and that the $0 trial-bookkeeping invoice still ACKs true (else: infinite retry loop). Cases C/D run the REAL dispatcher + handler with only stripe.invoices.retrieve stubbed. Needs MONGODB_URI — seeds/cleans real stripewebhookqueue rows. See docs/billing-stripe/STRIPE_WEBHOOK_QUEUE.md "The ACK gate".
