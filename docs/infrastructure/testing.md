...
npm run test:invoice-pi-recovery     # invoice PaymentIntent recovery (src/utils/payment/stripe/__tests__/invoice-payment-intent-recovery.test.ts): isInvoicePayable / requires_action confirmability / select-vs-drop split that keeps a void invoice from yielding a client_secret.
...
npm run test:claim-grant-compensation # a claim must never report success while granting nothing (src/services/redeemables/__tests__/claim-grant-compensation.test.ts). RedemptionService.redeem used to burn the issuance + stamp the permanent redeemedEverAt marker, then DISCARD the boolean from DrawGrantService.grantMonthlyCouponEntries and return success unconditionally — so in a freeze/gap window with no target draw the customer was told "200 free entries added", their one-per-lifetime grant was spent, and no draw received anything (the only trace was a console.warn, which production builds strip). Four scenarios: a monthly-coupon claim whose grant returns false reports grant_unavailable and reverses everything (issuance back to active, redeemedEverAt unset, wallet counter + redemptionHistory row undone); the same issuance then claims for real; a PRE-EXISTING redeemedEverAt (refunded legacy coupon) is preserved, not erased; and the manual milestone claim compensates identically when the grant throws. grantMonthlyCouponEntries is stubbed on the class object rather than deleting/freezing real draws, so the failure is forced without mutating shared draw data. NEEDS DB (.env.local) — creates and removes its own users/campaigns/issuances, and load-mutate-saves the draw on cleanup so MajorDraw.totalEntries stays consistent. See docs/rewards-redeemables/gotchas.md.
...
Added 2026-08-27: `npm run test:typed-code-checkout` (`src/utils/payment/__tests__/typed-code-at-checkout.test.ts`) — the purchase-click resolve for a discount code the customer typed but never applied. Pure (`globalThis.fetch` stubbed), no DB/env. Two things make it worth copying: it pins an **infrastructure-vs-refusal** distinction in both directions, because the endpoint under test returns the same `valid: false` for a genuine bad code, its own 429 and its own 500 — collapsing them would convert our rate limiter into refused sales, which `tsc` cannot see; and it **exercises the AbortController timeout for real** against a fetch that honours the signal and never settles otherwise, rather than asserting the constant. It closes with a rule-11 sweep over the assembled customer-facing copy. See `docs/payment/testing.md`.
...
Added 2026-09-04: `npm run test:one-time-charge` (`src/utils/payment/stripe/__tests__/resolve-purchase-payment-intent.test.ts`) — the one-time pack **double-charge** fence, using the `require.cache` stub-swap pattern with `@/lib/stripe` as the recorder (identity-checked before any case runs). Worth copying for one reason: **its load-bearing assertions are negative.** The defect was an extra `paymentIntents.create` call, so the test asserts that call does *not* happen when a confirmed charge already exists — a test written around the returned PaymentIntent would have passed for the entire nine months the bug shipped, because the return value was always correct. It also pins the invariant in **both** directions, which is the half that is easy to omit: a *claimed* charge must NOT be adopted, so a member deliberately buying the same pack twice is still charged twice — a fix that quietly under-charges is worse than the bug. Both best-effort paths (failed metadata stamp, failed recovery lookup) are asserted to degrade without either failing the sale or double-charging. The file closes with a source-level thread check across route schema → mutation destructure → request body → modal call site, explicitly labelled as weaker than driving the code (per the `test:campaign-code-metadata` rule that a grep pins nothing) and justified only because every way of losing that value is silent and the realistic regression is deletion. **Mutation-tested before being trusted:** disabling the resolver's adopt branch reproduces the original bug and turns the suite red. See `docs/payment/testing.md`.
