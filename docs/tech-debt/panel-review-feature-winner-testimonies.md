...
- [ ] **F-001** · ~~P0~~ → **LATENT / pre-launch blocker** (downgraded — see status) · Security · `src/app/api/auth/send-otp/route.ts:58,96` — SMS login code is sent to an attacker-supplied phone number.
      ...
      _Fix:_ In `send-otp/route.ts:96` send to `user.mobile` (400 if absent), and delete `mobile` from `sendOTPSchema` (`:9`) so the client can't supply a delivery target. _Handled:_ deferred to SMS-OTP launch prep.
