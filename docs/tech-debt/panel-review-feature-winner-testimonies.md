...
- [ ] **F-001** · P0 · Security · `src/app/api/auth/send-otp/route.ts:58,96` — SMS login code is sent to an attacker-supplied phone number.
      _What:_ `send-otp` resolves the account by **email** (`:58`), stores the OTP on it (`:88-91`), then sends the SMS to `validatedData.mobile` — the **request body's** number, not the stored `user.mobile` (`:96`). An attacker who knows a paying member's email POSTs `{email: victim, mobile: attackerPhone}`, receives the code, and `verify-otp` logs them in as the victim. Gated only on "has active membership" + a per-instance email-keyed rate limit. **Independent, pre-existing — NOT introduced by this diff.**
      _Fix:_ In `send-otp/route.ts:96` send to `user.mobile` (400 if absent), and delete `mobile` from `sendOTPSchema` (`:9`) so the client can't supply a delivery target. _Handled:_ —
...
- [ ] **F-004** · P2 · Security (residual, pre-existing) · `src/app/api/auth/register/route.ts:702-710` (+ `:465`, `:609`) — a non-staff plain customer's account can be taken over by mobile.
      _What:_ For a plain (non-privileged) account matched by **mobile only**, the route overwrites its `email` with the attacker's and does not reset `isEmailVerified`, so an attacker who knows a zero-entry customer's mobile can rebind the login email to themselves and sign in via email code. Low real-world value (empty target); disclosed in `docs/auth/gotchas.md`.
      _Fix:_ In the mobile-only branch, do not reassign `existingUser.email` when the submitted email matched no account (keep stored, or reject with a log-in 400). Additionally, in every update branch set `isEmailVerified=false`/`isMobileVerified=false` when that contact field changes. _Handled:_ —
...
- [ ] **F-005** · P2 · Security (pre-existing) · `src/app/api/auth/register/route.ts:416,457` — the pre-existing converted/saved-payment mobile-match rejections disclose the matched account's email the same way F-002 did.
      ...
      _Fix:_ Drop `existingAccountEmail`/`isExistingAccount` from the mobile-match converted and saved-payment-method rejection branches (mirror the F-002 fix). _Handled:_ —
