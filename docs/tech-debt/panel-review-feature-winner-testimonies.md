...
- [x] **F-003** · P1 · Test · **FIXED 2026-07-23** · `e2e/specs/auth/registration.spec.ts` — the guard's route integration is untested and the e2e seed lacks the critical staff shape.
      ...
      _Fix:_ Add `STAFF` creds to `e2e/helpers/db.ts:11`; seed a `{role:"user", userType:"staff", roleId:new ObjectId(), ...}` user in `e2e/seed/users.ts`; add two `@smoke` cases in `registration.spec.ts` — register with the staff email → 400 `field:"email"` + account unchanged; register with the staff mobile → 400 `field:"mobile"` + staff `email` unchanged. _Handled:_ **FIXED 2026-07-23** — added a `registration privileged-account guard @smoke` describe with a per-worker `role:"user"/userType:"staff"/roleId` seed + three specs (staff-email reject, staff-mobile reject + no-leak, plain-guest re-register overwrite preserved). Implemented in-spec (not the shared seed) to limit blast radius. `npm run e2e --grep "privileged-account guard" --project chromium-desktop` → **5 passed**.
...
- [x] **F-005** · P2 · Security · **FIXED 2026-07-23** · `src/app/api/auth/register/route.ts` — the pre-existing converted/saved-payment mobile-match rejections disclosed the matched account's email the same way F-002 did.
      ...
      _Fix (mirror F-002 exactly):_ Drop **only** `existingAccountEmail` from the mobile-match converted and saved-payment-method rejection branches — **KEEP `isExistingAccount: true`** so the "please log in" modal still opens for the returning customer (dropping it would downgrade them to an inline error — a UX regression). This is the same shape as the applied F-002 fix. _Handled:_ **FIXED 2026-07-23** — dropped `existingAccountEmail` from both mobile-match branches (converted + saved-payment), kept `isExistingAccount`. Verified the client falls back to `formData.email` ([MembershipModal/index.tsx:1614](../../src/components/modals/MembershipModal/index.tsx#L1614)) so legit customers keep a correct prefill. Covered by the F-003 mobile spec (asserts `existingAccountEmail` is `undefined`).
...
- [ ] **F-006** · P2 · Copy (optional) · `src/app/api/auth/register/route.ts:362,375` — the new 400 `message` strings are never displayed.
      ...
      _Disposition — **SKIP (won't-do), 2026-07-23**:_ the `message` is NOT fully dead — the client uses `message.includes("has made purchases")` as a fallback trigger for the ExistingAccountModal ([MembershipModal/index.tsx:1611](../../src/components/modals/MembershipModal/index.tsx#L1611)) — so editing these strings carries a subtle control-flow risk for **zero** user-visible gain (the string isn't rendered). Not worth it. _Handled:_ skipped by design.
