...
## R9. Dashboard access gates on `hasEverPaid` — never `subscription.isActive` (2026-08-27)

Middleware redirects a signed-in account with no purchase history off `/my-account` and `/rewards` to `/membership`. Gate it on [`hasEverPaid`](../../src/utils/auth/has-ever-paid.ts) — the single predicate for "is this a real customer?" — and import that rather than inventing another answer. `subscription.isActive` is the wrong flag: it is false for cancelled, paused and past-due members, so gating on it would shut **4,613 paying customers** out of their own dashboard, including past-due members still holding live draw entries. Two properties are load-bearing and must survive any edit: the condition is `=== false` (an absent stamp is allowed through — see architecture.md "Page redirects in middleware"), and the redirect response carries `buildSecurityHeaders(nonce)` + `x-nonce`, like the → `/login` and → `/` redirects it sits between. Middleware never runs for `/api/**` (R4), so this is a page gate only — an endpoint that must be customers-only calls the predicate itself.
