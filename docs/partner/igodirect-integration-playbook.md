...
- **`src/utils/partner-discounts/portal-return.ts`** (panel-fix F-003) — the pure rewards-return core: `resolvePortalReturn` (untrusted-URL parser; offers map dependency-injected so the client-shared module never imports the server-only map; prototype-key + all-digits guards on `offer_id`) and `resolvePortalBannerView` (the banner's six-state copy/CTA matrix). Tested via `npm run test:portal-return` (18 cases).
