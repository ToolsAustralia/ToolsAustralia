...
## ElectricPackageCard light-mode variant — 2026-05-16

`ElectricPackageCard` accepts an optional `theme?: "light" | "dark"` prop (default `"dark"`). When `theme="dark"` the card renders byte-for-byte identically to the original approved design; all existing consumers are unaffected. When `theme="light"` the card switches to a white/light-grey surface (`#ffffff → #eef0f3` body gradient), tier-accent border, soft accent glow shadow, and darkened accent ink (`accentInk = shade(accent, 0.58)`) for text, price, CTA border/text, divider, price panel, and swing tag — ensuring pale tiers (lime/gold/cyan) remain legible on light. The CTA button keeps a black background in both modes. The dark `isPremium` gold-gradient title and VIP glow paths are bypassed in light mode (plain `accentInk` text instead).
