...
**Inline two-Embla pattern (overlay UI on the main viewport, 2026-05-10):** [`FullscreenImageViewer`](../../src/components/ui/FullscreenImageViewer.tsx) and [`MiniDrawImageGallery`](../../src/app/(site)/mini-draws/[id]/components/MiniDrawImageGallery.tsx) (Phase 3 of the smoothness plan) keep `useEmblaCarousel` inline rather than using `EmblaThumbsGallery<T>` because both files need siblings rendered *between* the main and thumbs viewports — a captioned info bar in the fullscreen viewer, and absolutely-positioned navigation chevrons + pagination dots + counter overlaid on the rounded main card in the mini-draw gallery. The wrapper renders `<main /><thumbs />` as fixed siblings under one root, so neither pattern fits. The inline version uses the same option/plugin shape as `EmblaThumbsGallery<T>` (main: `loop: false, duration: 25` + `ClassNames()`; thumbs: `containScroll: "keepSnaps", dragFree: true` + `ClassNames()`) and replicates the linkage: `mainApi.on("select", onSelect)` updates the active index, scrolls the thumbs strip via `thumbsApi.scrollTo(i)`, and clicking a thumb calls `mainApi.scrollTo(i)`. Both viewports get `data-carousel="true"` and `touch-action: pan-y pinch-zoom` manually since the wrapper isn't wrapping them. Pagination dots are rendered from `mainApi.scrollSnapList()` length so dot count tracks slide count even on dynamic image arrays. Keyboard navigation (`ArrowLeft` / `ArrowRight` / `Escape`) is wired by the file's own `keydown` listener calling `mainApi?.scrollPrev()` / `scrollNext()` / `onClose()`. Pre-existing `swiper` references remain only in [`MajorDrawSection`](../../src/components/sections/MajorDrawSection.tsx) (Phase 4 will migrate it) and a stale `swiper/css` import in `PartnerHero` unrelated to this migration.
