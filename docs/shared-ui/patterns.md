...
- `RecentWinnersCarousel` was rebuilt on top of `useEmblaCarousel` (no Swiper, no manual `slice(currentIndex, currentIndex + itemsPerView)`, no `window.resize` listener — Embla observes container resize natively). Per-card markup moved into `RecentWinnerCard`, a memoised component that consumes the `RecentWinner` shape from the new TanStack hook. This was intentionally a separate component from the existing [`WinnerCard`](../../src/components/cards/WinnerCard.tsx) (which renders the `/winners` gallery card with promo-theme borders and a "View draw" CTA): the homepage carousel variant had a fundamentally different design — full-bleed photo, name overlay, draw-type badge — and the two components never shared JSX. Reach for `WinnerCard` for the `/winners` gallery and any other surface that wants a clickable "go to draw" affordance. (`RecentWinnersCarousel`, `RecentWinnerCard`, and the `useRecentWinners` hook below were removed in the 2026-07-01 dead-component sweep.)
...
[`useLeafTimer(intervalMs?)`](../../src/hooks/useLeafTimer.ts) is a `setInterval` re-render isolator: it owns the `now` state in a leaf component so a parent doesn't re-render every tick. The pattern is a small per-host leaf component that wraps `useLeafTimer` and renders just the ticking countdown — e.g. `FloatingCountdownLeaf` (in `FloatingCountdownBanner.tsx`), `GiveawayCountdownLeaf` (in `GiveawayCountdownTimer.tsx`), and `MajorDrawCountdownLeaf` / the `MiniDrawCountdown` leaf — so their hosting sections (`MajorDrawHeaderStrip`, `MajorDrawOverview`, etc.) don't re-render on every tick.
