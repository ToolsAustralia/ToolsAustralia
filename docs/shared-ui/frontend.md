...
> **One-time section header follows the pack type (2026-07-03):** the tier list's one-time section now reads its header off the actual packs — when the dashboard feeds it member (Additional) packs (`cta.oneTimePlans[0]?.isAdditional`), the eyebrow shows "**Additional packages**" / "**Member pricing**" instead of "One-time packages" / "No subscription". The pack list itself (`OneTimePacksGrid` → `PackCard`) is unchanged; it renders whatever `cta.oneTimePlans` carries, so the member's discounted price + the `getAdditionalPackDiscount` coupon badge now show for members. The switch to Additional packs is driven by the page's `useMembershipCardCta({ includeAdditionalForMembers: true })` — see [dashboard-account/frontend.md](../dashboard-account/frontend.md).
