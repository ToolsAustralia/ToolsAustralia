...
| `BonusCodeAudienceService.ts` | Read-only **forecast**, per trigger, of how many customers a webhook-minted bonus code can reach — plus current issued/redeemed counts. Never mints/issues/redeems. Behind `GET /api/admin/monthly-coupon/trigger-audience` (see [api.md](./api.md#get-apiadminmonthly-coupontrigger-audience--bonus-code-audience-forecast-2026-09-01)). Reads `BONUS_CODE_BY_TRIGGER` from `src/config/bonusCodes.ts`, never restates it. |
...
| `bonusCodeAudienceFilter.ts` | Pure Mongo-filter builders for `BonusCodeAudienceService` — the addressable-population "who could this trigger reach" query, one per trigger (`cancel-click` / `checkout-start` / `one-time-purchase`), plus the shared `notCurrentlyActiveSubscriberOr()` fragment. `checkout-start`'s builder is a documented **approximation** (no persisted "started checkout" event exists) — see the file header. |
