...
- [backend.md](./backend.md) — RedemptionService, CampaignService, DrawGrantService, TargetingService, MilestoneIssuance, the bonus-code webhook guards (`src/lib/bonus-code-webhook/`), `BonusCodeAudienceService` (admin audience forecast)
- [api.md](./api.md) — **`POST /api/bonus-codes/v1/issue`** (the Klaviyo bonus-code webhook: contract, status map, secret rotation, launch order), `/api/redeemables/**`, `/api/rewards/**`, `GET /api/admin/monthly-coupon/trigger-audience` (per-trigger audience forecast)
