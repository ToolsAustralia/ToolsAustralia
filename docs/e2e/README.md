...
| Landing countdown tiers (`@demo`) | `e2e/specs/marketing/landing-drawn-states.spec.ts` | Moves the active major draw's `drawDate` and reloads to prove each brand landing page swaps to its own `drawn-tomorrow` / `drawn-tonight` hero still. **Serial** — it mutates the shared seeded draw and restores it in `afterAll`. |
