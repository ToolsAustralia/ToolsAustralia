...
| Landing countdown tiers (`@demo`) | `e2e/specs/marketing/landing-drawn-states.spec.ts` | Moves the active major draw's `drawDate` and reloads to prove each prize combination swaps to its own `drawn-tomorrow` / `drawn-tonight` hero still. **Was skipped 2026-08-26 → 2026-09-03:** draw 10 deleted all 480 drawn-tier clips and 160 drawn-tier stills because each baked `& $5K CASH` into the artwork, a prize component draw 10 removed, so every assertion would have failed on a resolver *fallback* rather than a real regression. **Re-enabled 2026-09-03** when the replacement art shipped via `npm run convert:draw10-urgency-landing` (192 stills + 192 clips, both modes, 6 brands × 4 toolboxes). Assertions are written against the **URL grammar**, not the artwork, so they survived the swap untouched. **Two tests, one per viewport** (Playwright cannot rescale a video canvas mid-recording — proof-mode.md rule 4); join with `npm run e2e:proof:join`. **Serial** — it mutates the shared seeded draw and restores it in `afterAll`. |
...
| Draw-10 asset rollover (`@demo`) | `e2e/specs/marketing/draw10-assets.spec.ts` | Seven beats per viewport proving the draw-10 changes end to end: the `NTP/17808` permit, the **removal** of the $5,000 combo cash bonus, STIHL joining as the sixth power toolset (4 x 6 = 24 combinations), the "New" badge moving from HiKOKI to STIHL, the card repainting in STIHL orange, and `/promotions/stihl` serving its own light **and** dark landing art. Everything is derived from the live registries (`TOOLBOXES`, `TOOLSETS`, `NTP_NUMBER`) rather than literals, so a registry edit moves the assertions with it. The no-cash guard scans the **rendered** page with the demo overlay stripped out — that is what caught two composed `$5,000` strings a repo-wide grep had missed. **Two tests, one per viewport** (same canvas-rescale constraint as `landing-drawn-states.spec.ts`); join with `npm run e2e:proof:join`. **Serial**, and read-only — it mutates no seeded data. |
