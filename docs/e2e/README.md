...
| Landing countdown tiers (`@demo`) — **SKIPPED** | `e2e/specs/marketing/landing-drawn-states.spec.ts` | Moves the active major draw's `drawDate` and reloads to prove each prize combination swaps to its own `drawn-tomorrow` / `drawn-tonight` hero still. **Skipped since 2026-08-26 (draw 10):** all 480 drawn-tier clips and 160 drawn-tier stills were deleted because each baked `& $5K CASH` into the artwork, a prize component draw 10 removed. Every assertion here would now fail on a resolver *fallback* rather than a real regression, so the describe carries `test.skip(true, …)` with the reason. **Un-skip the day the replacement countdown art ships** — that is the branch's only outstanding asset set. **Two tests, one per viewport** (Playwright cannot rescale a video canvas mid-recording — proof-mode.md rule 4); join with `npm run e2e:proof:join`. **Serial** — it mutates the shared seeded draw and restores it in `afterAll`. |
