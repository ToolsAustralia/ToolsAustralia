...
| [src/data/supportChatFaqs.ts](../../src/data/supportChatFaqs.ts) | **Cobber chatbot** FAQ corpus (`getSupportChatFaqEntries`) — deflection + knowledge pack only, NOT the /faq page. **96 entries** as of 2026-08-27 (SMS sign-in troubleshooting authored as id 88 and renumbered to **94** on merging staging, whose merchandise-shop batch had independently claimed 88-93; ids 32/37/44 rewritten for the verified-contact requirement — id 44 previously asserted verification was optional AND that entries are granted regardless, the latter already false because campaign issuance defaults to `requiresEmailVerified`); the count is pinned by `npm run test:chat-faqs` — bump it deliberately, and re-run `build:chat-knowledge-pack` + `calibrate:chat-deflection` after any edit. **The knowledge pack has a hard token ceiling** (`npm run test:chat-knowledge`, **16,400**) whose comment says not to raise it. The draw-10 additions were trimmed to fit at 15,198 rather than bumping it; the staging merge then pushed the pack to ~16,160 and the ceiling was raised, because a merge is the wrong moment to choose which of two sets of customer answers to delete. **ids 77 and 78 are each duplicated** (pre-existing, present before this merge), so 96 entries carry 94 distinct ids |
