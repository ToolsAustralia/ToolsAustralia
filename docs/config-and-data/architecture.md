...
| [src/data/supportChatFaqs.ts](../../src/data/supportChatFaqs.ts) | **Cobber chatbot** FAQ corpus (`getSupportChatFaqEntries`) — deflection + knowledge pack only, NOT the /faq page. **90 entries** as of 2026-08-27 (id 88 added for SMS sign-in troubleshooting; ids 32/37/44 rewritten for the verified-contact requirement — id 44 previously asserted verification was optional AND that entries are granted regardless, the latter already false because campaign issuance defaults to `requiresEmailVerified`); the count is pinned by `npm run test:chat-faqs` — bump it deliberately, and re-run `build:chat-knowledge-pack` + `calibrate:chat-deflection` after any edit. **The knowledge pack has a hard token ceiling** (`npm run test:knowledge-pack`, 15,200) whose comment says not to raise it — the 2026-08-27 additions were trimmed to fit at 15,198 rather than bumping it |
