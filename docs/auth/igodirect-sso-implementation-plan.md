...
|---|---|---|---|
| 1 | **Product API real shape** | ✅ **RESOLVED 2026-07-21 by live probe.** The row previously here was WRONG (it claimed `/api/v2/products` + `Authorization: Token token={KEY}:{SECRET}`). The masterplan (:50) was right all along. Confirmed working: **`GET https://api.myrewards.com.au/app/api/v1/products`, BASIC auth, `country` + `user_id` both MANDATORY** (400 otherwise). Returns ~10 items; `page`/`per_page`/`offset` are **ignored** — it is **not a bulk feed**. Response carries `id, merchant_id, name, highlight, details, redeem_button_img, image_url, product_images, web_coupon, out_of_stock…`; the real deep link lives in **`redeem_button_img`**. | unblocked |
| 2 | Product API **creds** | ✅ **RESOLVED — the SSO keys ARE the Product API keys**, exactly as iGoDirect told DJ. `Authorization: Basic base64(IGODIRECT_CLIENT_ID : IGODIRECT_SSO_SECRET)` → `200`. No auth → `401`. **✅ Join key confirmed: the API's `id` matches our offers-CSV `ID` (8/10 sampled).** Probe: `scratchpad/probe-api.js` (read-only, capped, never logs the secret). | unblocked |
| 2b | **Obtaining a member's numeric `user_id`** | 🔴 **the one remaining blocker.** Our SSO `member_id` (`tools_reward_user`) is not accepted as `user_id`, and `/verifytoken` redirects to `…/users/{domain}/new_benefits` with no numeric id. Ask iGoDirect: how do we resolve one of our members to their `user_id`? And can a `category_id` sweep return the full catalogue (their own sample used `category_id=538`)? | bulk enrichment + offer feed |
