...
## Staff Activity (audit log — Team → Logs sub-tab)

20. Sign in as Admin. Open `/admin/team` → click the **Logs** sub-tab. Empty filter, no search. Expect to see recent mutation rows from your own actions in the previous tasks (Users edits, Promos toggles, etc.) — newest first.
...
23. Sign in as Customer Support (no `audit.view`). Confirm the **Logs** sub-tab inside the Team page is hidden, the Staff actions tab inside UserDetailModal is hidden, and a direct GET to `/api/admin/staff-activity` returns 403.
...
25. As Admin, change a role's permissions in Team → Roles. Confirm a `settings.edit` row appears in **Logs** immediately (force-restart still applies separately to affected staff).

## Mobile responsiveness (Discord-style master-detail)

26. On a phone-sized viewport (Chrome devtools, iPhone 12 / 390 px width), open Team → **Roles**. Expect only the role list to be visible. Tap a role — the editor slides in over the list, with a back-arrow in the editor header. Tap the back arrow — you return to the list. Editing the role name and saving still works.
27. On the same viewport, Team → **Staff**. Each staff row stacks: avatar + name + status badge above, role select + resend/remove actions below. Resize the viewport above the `sm` breakpoint (≥640 px) — the row collapses back to a single horizontal line.
28. On the same viewport, Team → **Logs**. Confirm the search input spans full width, filter chips wrap, and the row layout stays readable without horizontal scroll.
