...
| POST | `/api/stripe/cancel-subscription` | User-facing cancel (delegates to subscription/CancelSubscriptionService). **The only caller that passes `isMemberChurn: true`** (renamed from `mintBonusCode` on 2026-08-26), which is what makes the service emit the cancel-time `"Subscription Cancellation Requested"` Klaviyo event that starts the win-back flow. The admin cancel route and the past-due tier switch leave it at its `false` default — neither is churn. See [rules.md R2](./rules.md) |
...
| POST | `/api/stripe/create-one-time-purchase-existing-user` | Same, for existing user. Thrown confirm-time card declines → same 400 shape. **Success `data` also carries `appliedCodes: { referralCode, promoLinkCode, campaignCode }`** (added 2026-08-28) — the three code legs actually stamped onto the PaymentIntent, `campaignCode` being `resolveCodeForCheckout`'s answer rather than the body's. `SpecialPackagesModal` has no attach call to veto a stale label, so this report is the only thing that licenses a *"code applied"* line on its receipt. Additive; see [backend.md](./backend.md#and-create-one-time-purchase-existing-user-now-says-what-it-verified-2026-08-28) |
