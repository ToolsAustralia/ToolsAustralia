...
| `npm run test:renewal-grant-reconciler` | **The paid-but-not-granted detector.** 16 assertions against a seeded 2019 window on the dev cluster: a paid `subscription_cycle` with no `BenefitsGranted` is reported; one WITH its grant is not; `failed` cycles, non-`subscription_cycle` invoices and rows outside the window are not; the reported row carries `userId`/`amountPaidCents`/`chargedAt`; inserting the missing grant clears the gap (the "is the backfill done?" signal); a `dead` queue row is reported with its `lastError` while a `queued` one is not; and the orchestrator's totals + echoed window are correct. Fixtures are written with the raw driver so `createdAt` can be pinned past Mongoose's `timestamps: true`. Needs `MONGODB_URI`. |
