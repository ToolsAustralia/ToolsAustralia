...
- **The phone field is `user.phone` — NOT `user.phone_number`.** `phone_number` is the **v1.2** name and *also* the browser pixel's `ttq.identify({ phone_number })` name, so it looks correct from both sides. v1.3 silently ignores unknown `user` keys — no error, no Diagnostics warning — so sending `phone_number` drops the parameter entirely. This shipped and was caught only via Events Manager → EMQ: `CompleteRegistration` (our one Events-API-**only** event, and one that *requires* a mobile at signup) sat at **0% phone coverage** while Purchase/AddPaymentInfo showed 74%/91% purely off the browser `ttq.identify`. Verified against Segment's production v1.3 destination (`TTUser.phone`).
- **Hashed (SHA-256, via the shared `hashPII`)**: `user.email` (lowercase+trim), `user.phone` (E.164 first), `user.external_id`, plus `user.first_name` / `user.last_name` / `user.zip_code` (lowercase, **all** whitespace stripped). **Plaintext, NOT hashed**: `user.city`, `user.state`, `user.country` (lowercase, alphanumerics only — hashing them breaks matching just as silently as a wrong field name). **Raw**: `user.ttclid`, `user.ttp`, `user.ip`, `user.user_agent`.
- **Match signals**: the funnel route [`/api/tracking/conversion`](../../src/app/api/tracking/conversion/route.ts) reads `ttclid` (first-party cookie set on ad-click landing by [`captureTikTokClickId`](../../src/utils/tracking/tiktok-helpers.ts)) and `_ttp` (the pixel's own cookie) via `extractTikTokContext`. The browser pixel also auto-attaches both. The **webhook-fired Purchase has no cookies**, so the payment-creation routes stash both in Stripe metadata as `capi_ttclid` / `capi_ttp` (alongside Meta's `capi_fbc` / `capi_fbp`); `extractRequestContextFromMetadata` reads them back and they ride on `requestContext` into `trackPixelPurchase`. TikTok additionally **merges** the browser copy with the server copy on the shared `event_id`.
- **Identity**: [`ConversionPixelsAdvancedMatching`](../../src/components/tracking/ConversionPixelsAdvancedMatching.tsx) calls `ttq.identify({ email, phone_number, external_id })` on login — `phone_number` is correct **here**, because the pixel SDK and the Events API genuinely use different names for the same field. The SDK hashes client-side using the SAME normalization (`normalizePhoneE164`, lowercase email) as the server, so hashes match.
...
reconcilePersistedAttribution(input: {
  ...
  persistedTouchAt?: number | null;            // epoch ms the persisted touch happened (session → now, signup → user.createdAt)
  now?: number;                                // epoch ms of the conversion; when provided, enforces the recency windows
...
3. **Edge `=== "direct"`** → recover a persisted **OWNED-channel** (`klaviyo_email`/`klaviyo_sms`) touch via `normalizeUtmToPlatform` + `isOwnedChannel` — **but only when the touch is within the channel's recency window** (see below).
4. **Edge `=== "direct"` + a persisted PAID-platform UTM (added 2026-07-19)** → recover it **strictly**: only when the touch is *affirmatively* inside the platform's click window — `persistedTouchAt` AND `now` both known, `0 <= age <= windowDays` (7d for meta/tiktok/snapchat/google), platform present in `PLATFORM_PRIORITY` (so `other` never recovers). The old stance ("an in-window paid click or cookie-visible signal already won at the edge, so edge `direct` means no paid touch") assumed the durable cookie is always present at PI-creation time — it is not (in-app-browser signup → external-browser checkout, Safari ITP, cookie blockers). Measured leak before the fix: **272 direct-bucketed purchases (~$9.0k) in 30 days carried a paid `utm_source`**, including same-session signup→purchase conversions off Meta retargeting ads. **Caller dating rule (load-bearing):** `payment-processing.ts` passes `persistedTouchAt = resolveSignupTouchAtMs(signupAttribution.visitedAt, user.createdAt)` for **signup**-sourced UTMs — the **captured ad-visit time**, with account creation only as the legacy fallback (2026-07-19: anchoring on account age buried returning members converting off BOF retargeting ads — e.g. a 196-day-old account whose ad visit was 62 seconds before purchase; registration refreshes `signupAttribution` in place, so `visitedAt` tracks the latest captured visit while `createdAt` stays frozen). **Session**-carried UTMs pass `null` — the client payload prefers the 90-day first-touch cookie with `capturedAt` stripped, and renewals re-carry frozen subscription metadata, so a session UTM's age is unknown and dating it `now` would fabricate freshness (stale-cookie purchases and every renewal would flip). Net effect: paid recovery fires only for purchases within 7d of a captured paid **ad visit**; stale or undatable paid UTMs stay `direct`. Known bounded over-credit: an organic re-registration that captures a still-live (<90d) cookie UTM gets `visitedAt` = registration time; fully fixing that requires persisting the cookie's own `capturedAt` into Stripe metadata (future work).

**Owned-channel recency window (added 2026-06-30):** the recovered owned touch is only credited when it is recent enough to plausibly have driven the purchase, using `windowDaysFor(platform)` from [`platformPriority.ts`](../../src/services/attribution/platformPriority.ts) — **5 days for Klaviyo email/SMS**, the SAME single source of truth the cookie resolver enforces. Recency is checked against the anchor the caller supplies:
- **Session**-carried UTM (`data.attributionSource === "session"`) → `persistedTouchAt = null` (undatable — see the paid-recovery note above; owned channels still credit via the lenient null rule, so Klaviyo behavior is unchanged from the previous `now` dating).
- UTM carried from **signup** → `persistedTouchAt = resolveSignupTouchAtMs(signupAttribution.visitedAt, user.createdAt)` — the captured visit time, `createdAt` only for legacy records without `visitedAt`. (Also applies to owned/Klaviyo recovery: a returning member whose refreshed signup UTM is a fresh Klaviyo link visit now credits correctly instead of being aged by their account date.)

Fallbacks (owned channels): `now` omitted → windowing disabled (legacy / back-compat, always counts); `persistedTouchAt == null` → recency unknown → credited rather than buried; a future touch (`age < 0`, clock skew) → out-of-window → `direct`. **Paid platforms invert every missing-data fallback**: `now` omitted, `touchAt` unknown, or no window row → NOT recovered (`isAffirmativelyWithinPaidWindow`).
...
**Historical reconcile:** [`scripts/backfill-klaviyo-attribution-cycle.ts`](../../scripts/backfill-klaviyo-attribution-cycle.ts) applies the **same windowed function bidirectionally** to already-saved rows — promoting in-window owned touches to Klaviyo and demoting now-stale ones back to `direct`. For the paid rule there is [`scripts/backfill-paid-attribution-recovery.ts`](../../scripts/backfill-paid-attribution-recovery.ts) (`backfill:paid-attribution` / `:dry`) — **up-credit only**, flipping historical `direct` rows to their paid platform when they qualify under the signup-anchored strict-window rule, using the same function and the same dating (session-carried → null → never flips). See `docs/infrastructure/architecture.md` for flags and the first prod dry-run numbers.
