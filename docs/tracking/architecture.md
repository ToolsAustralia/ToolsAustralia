...
Contentsquare UX analytics is loaded via `next/script` with `strategy="lazyOnload"` from [`src/app/layout.tsx`](../../src/app/layout.tsx) — browser idle *after* the load event, per the [R7 timing policy](rules.md#r7-third-party-script-timing-policy-2026-07-19-perf) (it is the single heaviest third-party script, ~157 KB gz). It is env-gated on `NEXT_PUBLIC_CONTENTSQUARE_ID` (R8), so it renders nothing in dev/e2e. Klaviyo and GTM also use `next/script` ([`KlaviyoScriptLoader.tsx`](../../src/components/KlaviyoScriptLoader.tsx), [`GoogleTagManager.tsx`](../../src/components/GoogleTagManager.tsx)). _(This paragraph said `afterInteractive` until 2026-08-03; the strategy actually changed with the 2026-07-19 perf work.)_

The tag alone only ever reports the **initial document load** — it has no History API detection, so in an App Router SPA an entire visit collapses into one pageview. [`ContentsquarePageTracker`](../../src/components/tracking/ContentsquarePageTracker.tsx), mounted from the root layout behind the same env gate, closes that: it pushes `["trackPageview", <path>]` on every `usePathname()` change (skipping the first, which the tag emits itself), and pushes `["excludeURLforReplay", <regex>]` once on mount to stop session-replay capture on the internal/staff surfaces. Both the replay-exclusion regex and the pageview suppression derive from the single `EXCLUDED_TRACKING_PREFIXES` list in [`should-track-route.ts`](../../src/utils/tracking/should-track-route.ts), so replay exclusion can never drift from pixel exclusion. See [frontend.md](frontend.md#contentsquare-spa-pageviews--replay-exclusion-2026-08-03) for the command contract.
