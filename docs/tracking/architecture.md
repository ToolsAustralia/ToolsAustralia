...
Contentsquare UX analytics is loaded via `next/script` with `strategy="lazyOnload"` from [`src/app/layout.tsx`](../../src/app/layout.tsx) — deferred to browser idle *after* the load event, because at ~157 KB gz it is the single heaviest third-party script and at `afterInteractive` it competed with hydration on low-end phones. The accepted trade-off is that session recordings miss the first ~2–4 s of a visit. This is the "analytics / marketing suites" class in [rules.md R7](rules.md#r7-third-party-script-timing-policy-2026-07-19); conversion pixels deliberately stay earlier. (This paragraph claimed `afterInteractive` until 2026-08-07 — it had been stale since the 2026-07-19 perf work moved the tag. R7 was right; this was wrong.) Klaviyo and GTM also use `next/script` ([`KlaviyoScriptLoader.tsx`](../../src/components/KlaviyoScriptLoader.tsx), [`GoogleTagManager.tsx`](../../src/components/GoogleTagManager.tsx)).

## Contentsquare client-side surface (`window._uxa`)

The tag (`t.contentsquare.net/uxa/<id>.js`, `lazyOnload`) is only half the integration — everything else is pushed onto the `window._uxa` command queue by two client components. Pushing before the tag has loaded is safe: the UXA tag does `window._uxa = window._uxa || []` on init and re-queues commands while its command service starts.
...
**One source of virtual pageviews, always.** Contentsquare can also emit them tag-side via a CSTC Artificial Pageview + HistoryChange snippet, and it does not de-duplicate — see [rules.md R11](rules.md#r11-contentsquare-virtual-pageviews-come-from-the-client-push-only--never-also-from-a-tag-side-cstc-snippet).

**Dynamic variables are scoped to the current pageview, not the session.** Every SPA navigation starts a fresh pageview that carries no variables, so the component re-pushes on every `pathname` change; Contentsquare keeps only the last value per key per pageview, so a repeat push is harmless. `identify` / `addUserProperties` are deliberately **not** used — both require the Product Analytics (heap) configuration this account does not have, so they would silently no-op. No PII is sent: tier and status are plan metadata.

**Privacy commands must be queued before the main tag fires.** Contentsquare's docs are explicit that `setPIISelectors` has to be pushed before the main tag; `excludeURLforReplay` has the same ordering requirement or a visitor landing directly on an excluded route is recorded before the rule applies. Both are pushed from an effect declared above the pageview effect, and the tag is `lazyOnload` (browser idle, after load), so the ordering holds comfortably. Masking mechanics: [frontend.md](./frontend.md#contentsquare-pii-masking--data-cs-mask-2026-08-07).
