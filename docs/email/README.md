...
Transactional email via SendGrid; HTML templates at repo root; transactional SMS via Mobile Message (Klaviyo owns marketing SMS); newsletter integration with Klaviyo (Klaviyo lives in [tracking](../tracking/) for marketing flows).
...
- [backend.md](./backend.md) — `lib/email`, template rendering, **`lib/sms` (Mobile Message adapter)**
- [api.md](./api.md) — `/api/newsletter/`
- [rules.md](./rules.md) — provider split (SendGrid transactional, Klaviyo marketing, Mobile Message transactional SMS)
- [patterns.md](./patterns.md) — template lockstep with code
- [gotchas.md](./gotchas.md) — preview workflow, root template files, SMS silent-drop traps
