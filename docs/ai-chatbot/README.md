...
> **Status:** Research + specification (no chatbot code exists yet — greenfield on branch `feature/ai-chatbot`).
> **Audience:** Tools Australia engineering, product, and the business owner.
> **Date:** 2026-06-22.
> **2026-06-24 final-review fixes:** costGuard NaN guard added — non-numeric `CHAT_DAILY_TOKEN_BUDGET_USD` now falls back to $5 default (regression test added). `chatStorage.ts` moved from `src/components/support-chat/` to `src/lib/support-chat/` (shared utility layer). `SupportChatWidget` z-index now derived from `Z_INDEX.MODAL_BASE - 1000` instead of a hardcoded 9000. `clearSupportChatStorage()` wired at all 4 sign-out sites (Header, settings/page, AdminSidebar, queries.ts forced-auth-error path) per the org privacy rule. Runbook §3c corrected: Vercel picks the most-specific glob regardless of order. Runbook §5 notes that kill-switch and tripped daily budget also gate FAQ deflection.
...
| 5 | **Website integration** | **Same-origin React widget** mounted in `src/app/(site)/layout.tsx` next to `UnifiedModalManager`; **no CSP change needed** (avoid a third-party script). Member-aware via NextAuth session; works for logged-out visitors too (FAQ-only). | High |
...
| — | **AU residency / privacy** | For onshore inference under the Privacy Act, use **Amazon Bedrock `ap-southeast-2` (Sydney)** — *not* the first-party API and *not* Vertex (neither offers AU-resident Claude). Phase 1 (logged-out, zero-PII FAQ bot) sidesteps the issue entirely and is the fastest compliant launch. | High |
...
These are the only material uncertainties left after verification. None blocks Phase 1 (logged-out FAQ bot); they matter for Phase 2 (authenticated per-user tools) and the residency decision.

1. **Haiku 4.5 in Bedrock `ap-southeast-2`** — confirm the triage model is available in-region. If not, options: (a) run Sonnet-only on Bedrock Sydney (still ~$0.15 AUD/conversation — cheap), (b) use Haiku via an offshore region for *non-PII* triage with APP-8 consent, or (c) wait for in-region availability. This is the single most important pre-build check.
2. **Whether AU onshore residency is required, or APP-8 informed consent is acceptable.** A business/legal decision. Phase 1 (no PII) avoids it; Phase 2 needs it resolved. Run a Privacy Impact Assessment before Phase 2.
...
Enforced **structurally** (not just by prompt), mirroring Norm's read-only discipline — full detail in [implementation-spec.md](implementation-spec.md) and [research.md](research.md) § Security:
...
- **Every per-user tool is scoped to the authenticated session `userId` resolved server-side** — the model can never supply a `userId`/email to widen scope (the classic cross-user data-leak path).
- **Output is projected to a tight PII boundary** (mirroring Norm's `firstName` + opaque `userId`), Zod-validated before it reaches the model or the user.
