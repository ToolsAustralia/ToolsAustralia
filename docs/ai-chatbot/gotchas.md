...
- **Abstain gate** ([faqSearch.ts](../../src/services/support-chat/deflection/faqSearch.ts)): `MIN_CONFIDENCE` 0.15 → 0.18 (2026-06-27) → **0.46** (2026-06-29, calibrated), **plus a top1-vs-top2 `MIN_MARGIN` (0.04)** — two near-tied candidates above the floor mean the query is ambiguous, so abstain to the grounded LLM rather than serve a coin-flip. Deflection is deliberately high-precision: a missed deflection costs one cheap (grounded) LLM call; a wrong deflection has no model in the loop. The 0.46/0.04 values were calibrated on the 96-case routingGoldenSet (0 mis-routes, 45 correct deflections — see `npm run calibrate:chat-deflection`).
