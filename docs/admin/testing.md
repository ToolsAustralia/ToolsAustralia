...
| `npm run test:attempt-spacing` | `src/server/admin/__tests__/attempt-spacing-alerts.test.ts` | **Regression guard (2026-08-24):** the PROACTIVE per-invoice attempt cap and run alerting. Cap: an invoice never attempted is charged immediately (the 229 starved members); every point inside the 3-day window is refused; the boundary is inclusive at exactly 3 days; `daysRemaining` never reports 0; the Mongo cutoff and the predicate window agree (if they drift, rows fall outside the query and the rule silently stops firing); and a 30-day walk yields **10** submissions per invoice, against the 24 measured before it existed. Bucketing: the `attempt_spacing` message buckets as itself — verified by mutation that removing the branch drops it into `other` — while every sibling skip message still buckets correctly, and `eligible = attempted + skipped` still balances with held rows. Alerts: the low-rate floor fires on all five real pre-fix runs and on the 2026-06-29 $0-collection shape, stays silent on a healthy run and on a sub-50-attempt run, always fires on `aborted`, never judges a `running` run, and never throws on legacy totals. |
