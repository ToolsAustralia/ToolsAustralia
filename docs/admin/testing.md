...
| `npm run test:attempt-spacing` | `src/server/admin/__tests__/attempt-spacing-alerts.test.ts` | **Regression guard (2026-08-24):** the PROACTIVE per-invoice attempt cap and run alerting. Cap: an invoice never attempted is charged immediately (the 229 starved members); every point inside the effective window is refused, 24h and 48h included; the 2h grace lets a day-3 touch arriving 45 min early proceed instead of slipping to day 4, while never reaching far enough to admit two submissions in 24h; the Mongo cutoff stays at least as WIDE as the predicate window (narrower would drop boundary rows out of the query and silently stop the rule firing); `daysRemaining` never reports 0; and two 30-day walks — fixed run times and drifting ones — both yield **10** submissions per invoice, against the 24 measured before it existed. Bucketing: the `attempt_spacing` message buckets as itself — verified by mutation that removing the branch drops it into `other` — while every sibling skip message still buckets correctly, and `eligible = attempted + skipped` still balances with held rows. Alerts: `zero_coverage` fires on the cap's own collapse mode — a run that finalizes `completed` having held every item, which `aborted` and `low_success_rate` both structurally miss — while staying silent on a genuinely empty worklist; the low-rate floor fires on all five real pre-fix runs and on the 2026-06-29 $0-collection shape, stays silent on a healthy run and on a sub-50-attempt run; `aborted` always fires and pairs with `zero_coverage` when a run is swept before its first chunk; a `running` run is never judged; legacy totals never throw. Mutation-verified: zeroing the grace and deleting the `zero_coverage` branch each fail the suite. |
...
| `npm run test:dashboard-stats-window` | `src/services/admin/dashboard-stats/__tests__/slidingWindowExcludesToday.test.ts` | The sliding window contains only COMPLETE AEST days, and `writeSnapshotForDate` refuses a day that has not closed — the 2026-08-25 partial-day-snapshot fix. Covers `aestPreviousDateKey` across month/year/DST boundaries |
| `npm run test:dashboard-stats-ad-restatement` | `src/services/admin/dashboard-stats/__tests__/adChannelRestatementWindow.test.ts` | **Meta quota guard (2026-09-01).** `resolveAdChannelsForDate`'s three branches, with providers and the stored-snapshot read injected (zero-DB, zero-network): a day **inside** the restatement window fetches and writes the live value even when something is stored; a **settled** day with usable stored `adChannels` does **not** fetch and its stored value is written through unchanged; a settled day with **no** usable stored value (absent row, empty map, or a channel that isn't the one being asked about) **fetches anyway** — the incident-shape guard; a fetch **error** still preserves the prior stored value (`preserved`) and leaves a channel with nothing to preserve absent rather than zeroed (`lost`); the backfill's no-window mode fetches everything. Pins the window constant at 10 with its `DASHBOARD_STATS_AD_RESTATEMENT_WINDOW_DAYS` override (empty/garbage/`0` → default) and its DST-correct boundary. Asserts the reduction as a number: over a 90-day window, **10 fetched / 80 reused → 270 → 30 Meta calls/day**. **Mutation-verified:** replacing the source with the naive "skip every out-of-window day" implementation fails the branch-3 assertions (fetch count 1 → 0, `adChannels` `{...}` → `{}`) while tests 1, 2 and 5 still pass. See [gotchas.md](./gotchas.md#only-the-newest-10-days-are-re-fetched-from-the-ad-providers-2026-09-01). |
