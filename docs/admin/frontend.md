...
- **[src/app/admin/component/overview/sections/KpiGrid.tsx](../../src/app/admin/component/overview/sections/KpiGrid.tsx)** — replaces the old `KPIMetricsGrid` at the top of `DashboardOverview`. Pure presentation: receives `stats` (`AdminDashboardStats`), `membership` (`MembershipByPackageData`) and `dateRange` as props (fetched in `DashboardOverview`, not here). Renders two labelled groups:
  ...
  - **Users & Performance** (`grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5`): Total Users (indigo), New Signups (blue), Conversion (violet), **Renewals** (emerald, always rendered), Cancellations (red, `invert`). _(Renewals/Cancellations order swapped 2026-06-03 — Renewals 3rd, Cancellations 4th.)_
...
- **`PlatformHourlyRevenueSection`** (`src/components/admin/`) — shared hour-of-day section for an ad-platform tab. Owns a default last-30-day **AEST** range, calls `useHourlyRevenue({ platform })` (SHARED-1 endpoint, which now also returns per-hour **spend**), and renders four `MetricCard`s (Ad Spend, Attributed Revenue, Conversions, ROAS) + a 24-row `DataTable` with **Hour / Spend / Revenue / Profit / ROAS / Conv.** columns (profit = revenue − spend, ROAS = revenue ÷ spend). Where a platform has **no spend source** (Snapchat, or TikTok before its creds), Spend/Profit/ROAS render **"—"** (not 0) and the subtitle notes spend arrives with the Marketing-API sync. Used by `TikTokAdsManagement` (`platform="tiktok"`) and `SnapchatAdsManagement` (`platform="snapchat"`). **TikTok spend** comes from `src/services/admin/tiktok/tiktokHourlySpend.ts` (graceful — inert without `TIKTOK_ADVERTISER_ID`/`TIKTOK_MARKETING_ACCESS_TOKEN`; **unverified against the live API** until creds exist).
