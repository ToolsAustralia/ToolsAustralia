...
- Attributed revenue in AUD with optional `revenueTrend` chip (arrow + %)
- Confidence split: `$X click · $Y estimated` where estimated = `utm_only + inferred_backfill`
- Conversion count
- True ROAS (e.g. `2.14x`) with optional `trueRoasTrend` chip — only rendered when `trueRoas` is present (i.e. the platform has ad spend data)
