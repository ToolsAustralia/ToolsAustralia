...
**Kept** (still in use): `overview/DashboardOverview.tsx`, `overview/OverviewToolbar.tsx`, `overview/DashboardSection.tsx` (used by `UsersBreakdownSection`), `overview/UsersBreakdownSection.tsx`, and all `overview/sections/*` redesign cards. The `my-account` `RecentOrders.tsx` (a different file) was untouched by the Overview reskin, though it was subsequently removed in the 2026-07-01 dead-component sweep (the admin-side `RecentOrders.tsx` it is contrasted with here remains an orphan-dead file — see the barrel note above). Detail modals the deleted sections used (`RevenueDetailModal`, `MembershipByPackageDetailModal`, `PrizePerformanceAdsModal`) were left in place — they live under `src/components/admin/` and are out of scope for the Overview reskin.
