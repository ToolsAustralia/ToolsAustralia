...
- **[src/app/admin/component/overview/sections/KpiGrid.tsx](../../src/app/admin/component/overview/sections/KpiGrid.tsx)** — replaces the old `KPIMetricsGrid` at the top of `DashboardOverview`. Pure presentation: receives `stats` (`AdminDashboardStats`), `membership` (`MembershipByPackageData`) and `dateRange` as props (fetched in `DashboardOverview`, not here). Renders two labelled groups:
  - **Revenue** (`grid grid-cols-2 lg:grid-cols-4`): Revenue (emerald, clickable), **MRR** (red, clickable — formerly "Membership Revenue"), Ad Spend (blue), ROAS (green).
  - **Users & Performance** (`grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5`): Total Users (indigo), New Signups (blue), Conversion (violet), Cancellations (red, `invert`), **Renewals** (emerald, always rendered).
  ...
  - **Per-card date tag (2026-06-02):** date-filtered tiles append a `rangeLabel` parenthetical to their title — e.g. `Revenue (Today)`, `Ad Spend (Yesterday)`, `ROAS (Last draw name)`, `Renewals (Nov 2025 – present)`. Tagged tiles: Revenue, Ad Spend, ROAS, New Signups, Conversion Rate, Cancellations, Renewals. **Untagged** (value not scoped to the filter): MRR (current snapshot) and the all-time Total Users variant. `KpiGrid` receives the resolved string via a new `rangeLabel?: string` prop; `DashboardOverview` computes it (today/yesterday literals, draw `name` from `useCurrentAndLastDrawDates`, all-time `${format(stats.dateRange.start,"MMM yyyy")} – present` using the Nov 2025 website-launch start, custom `MMM d – MMM d`). The title CSS is `truncate`, so long draw names ellipsize rather than overflow.
...
- **`PlatformHourlyRevenueSection`** (`src/components/admin/`) — shared hour-of-day revenue section for ad platforms with **no ad-spend sync yet**. Owns a default last-30-day range, calls `useHourlyRevenue({ platform })` (SHARED-1 endpoint), and renders four `MetricCard`s (Ad Spend "—"/Awaiting sync, Attributed Revenue, Conversions, ROAS "—"/Needs spend) + a 24-row `DataTable` (Hour AEST / Revenue / Conversions). Used by `TikTokAdsManagement` (`platform="tiktok"`) and `SnapchatAdsManagement` (`platform="snapchat"`), which replaced their em-dash empty shells.
...
- **`KlaviyoAnalyticsManagement`** (`src/components/admin/`) — new **Klaviyo** tab (Analytics group, gated `facebookAds.view`). Balanced layout: (1) a "scheduled / about to send" strip (upcoming Scheduled campaigns + live Flows), (2) **Campaigns** + **Flows** revenue tables ranked by **Klaviyo-attributed** revenue with an **email / SMS** split, (3) the **server-side** Klaviyo hourly revenue (`useHourlyRevenue({ platform: "klaviyo" })`, SHARED-1). Data via `useKlaviyoAnalytics` → `GET /api/admin/klaviyo/analytics` (SHARED-2). A footnote makes clear the campaign/flow revenue is Klaviyo's own attribution and won't equal the server-side `convertingPlatform=klaviyo_*` hourly. No range selector in v1 (defaults last-30-days); no auto-refresh (Klaviyo reporting is throttled).

## All-Platforms aggregate tab (Part D, 2026-06-03)

- **`AllPlatformsManagement`** (`src/app/admin/component/`) — new **All Platforms** tab (first in the Analytics group, gated `facebookAds.view`). **Ad-effectiveness only (renewals excluded).** KPI rollup from **`computeAggregate(stats.attributedRevenue)`** (SHARED-3, client-side, unit-tested in `advertisingCardModel.test.ts`): Total Ad Spend, Attributed (acquisition) Revenue, **Overall ROAS** (paid channels with spend ÷ their revenue — mirrors the overview card's blended ROAS so they reconcile), **Contribution** (`revenue − ad spend`, sign-aware), Conversions. Below: the reused **`AdvertisingPlatformCard`** for the per-platform breakdown, and the server-side hour-of-day table (`useHourlyRevenue({ platform: "ad-channels" })`, SHARED-1 — the 5 ad channels, matching the KPI scope so the two reconcile). Data via `useAdminDashboardStats("custom", …)` over a default last-30-day range. All-source/total (incl. renewals) revenue stays on the Overview revenue card.
