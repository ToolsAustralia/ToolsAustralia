...
- **[src/app/admin/component/overview/sections/KpiGrid.tsx](../../src/app/admin/component/overview/sections/KpiGrid.tsx)** — replaces the old `KPIMetricsGrid` at the top of `DashboardOverview`. Pure presentation: receives `stats` (`AdminDashboardStats`), `membership` (`MembershipByPackageData`) and `dateRange` as props (fetched in `DashboardOverview`, not here). Renders two labelled groups:
  ...
  - **Users & Performance** (`grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5`): Total Users (indigo), New Signups (blue), Conversion (violet), Cancellations (red, `invert`), **Renewal Rate** (emerald, always rendered).
  ...
  - **Renewal Rate tile (redesigned 2026-06-02):** headline value is the **current billing cycle** rate, independent of the selected date range filter. Rendered as `{rate}%` with `valueAside="{renewed}/{base} renewed"` (using `MetricCard`'s new optional `valueAside` prop — small muted text inline next to the big value). The cycle is anchored to the last completed `MajorDraw.drawDate` + 1 day (AEST, via `aestDayBounds(...).dayEndUTC`) through to now; the numerator and denominator come from `stats.users.renewalProgress` (always populated — see backend.md). The **sub-line** is a filter-driven slice showing `{rangeLabel}: {renewed} renewed · {pastDue} past due`, drawn from `stats.users.membershipRenewals.succeededDistinctMembers` and `stats.users.membershipRenewals.becamePastDueInRange` for the selected date filter.
