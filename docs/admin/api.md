...
| `/api/admin/products/**` | [cart-shop-products](../cart-shop-products/api.md) | Shop catalog CRUD, gated on `shop.view` / `shop.edit` / `shop.delete`. **These are the only single-product create/update paths in the codebase** — the public `/api/products/**` family has no `POST /api/products`, only a bulk `import` and a `duplicate/[id]`. Delete is its own permission because the public family also carries bulk-destruction routes. |
...
| `GET/POST/PUT/DELETE /api/admin/monthly-coupon/campaign[/[id]]` | [rewards-redeemables](../rewards-redeemables/) | Redeemable-campaign list/create/update/delete. **Validation (2026-07-06):** `manual-users`/`csv-users` targeting requires a non-empty `segmentConfig.includeUserIds` — create rejects via zod `superRefine`; update validates the MERGED state in `CampaignService.updateCampaign` (PUT is partial). **`validForHours` (2026-08-25):** optional per-customer rolling-window field, mutually exclusive with `neverExpires` on both create and update (zod `superRefine`, since `updateCampaign`'s `findByIdAndUpdate` skips the model's `pre("save")` guard). On update, `null` is the clearing sentinel (`.nullable().optional()`) — a bare omission never clears it, since two independent layers already strip `undefined`. **Open-ended `endsAt` (2026-08-27):** the `validForHours` contract is **unchanged**, but `endsAt` may now legitimately carry the far-future sentinel `9999-12-31T23:59:59.999Z`, meaning "this campaign has no minting backstop and keeps issuing until an admin disables it" (`neverExpires` stays `false`). The admin form writes it for always-on trigger campaigns. No route validation changed — it is a valid ISO date that passes every existing check. Any consumer formatting `endsAt` for display should test it with `isOpenEndedDate` from `src/utils/redeemables/bonus-code-policy.ts` (a **year threshold**, never an equality test) and render "no end date" rather than a year-9999 calendar date. `GET` now also returns `issuanceCount` (total `RedeemableIssuance` rows, any status) per campaign. `DELETE` is now **always** a soft delete (`isActive: false`) — see [rewards-redeemables/api.md](../rewards-redeemables/api.md#validforhours-contract-createupdate-campaign). See [rewards-redeemables/gotchas.md](../rewards-redeemables/gotchas.md). |
| `GET /api/admin/monthly-coupon/trigger-audience` | [rewards-redeemables](../rewards-redeemables/) | **Bonus-code status (2026-09-01, reworked).** Read-only — never mints/issues/redeems. Gated `rewards.view`. Per webhook-minted trigger (`cancel-click`/`checkout-start`/`one-time-purchase`), returns PRIMARY real `RedeemableIssuance` state (issued / still-redeemable — via `isCampaignRedeemable`, never hand-rolled / redeemed+entries / expired-or-lapsed, each with a bounded 25-row sample) plus a SECONDARY, demoted addressable-population forecast (all three campaigns sit at 0 issuances in production, so the primary numbers render 0 today — by design). Delegates to `BonusCodeAudienceService`. Full contract: [rewards-redeemables/api.md](../rewards-redeemables/api.md#get-apiadminmonthly-coupontrigger-audience--bonus-code-status-2026-09-01-reworked). |
...
- `detail` — campaign → ad-set → ad tree per bucket from the **live** platform-insights × `AdDestination` join (nodes sorted by spend desc at every level). Bounded by the insights TTL (~60d prod): `availableSince` = the account's oldest retained insights date (unbounded indexed lookup, range-independent), `complete = availableSince <= startDate`. A zero-activity range with the floor before it is `complete: true` with empty buckets.
- **`platform=tiktok` is fully supported as of 2026-07-29** and returns real buckets. `supported: false` now carries `reason: "not-configured"` and means only that this environment has no account id for the requested platform — an unconfigured platform is **not** a `500`, and the modal states explicitly that it differs from "$0 spent". The account id is resolved per platform (`FACEBOOK_AD_ACCOUNT_ID` vs `TIKTOK_ADVERTISER_ID`) by `src/services/analytics/adPlatformAccounts.ts`.
- Reading TikTok's numbers: its **one-time bucket is legitimately `$0`** (every TikTok ad points at a membership landing page — campaign setup, not a classification failure), and its `availableSince` is far more recent than Meta's because TikTok insights only began syncing in July 2026, so `complete: false` on longer ranges is expected. On-read freshness is **Meta-only**; TikTok's rollup is rebuilt nightly by `/api/cron/sync-tiktok-ads`, which runs the full insights → destinations → aggregate pipeline (it was insights-only before 2026-07-29, which left TikTok's rollup permanently empty in production with nothing appearing to fail).
...
Thin handler — delegates to `getKlaviyoAnalytics(range, nowMs)` in `src/services/admin/klaviyo/klaviyoReporting.ts` (SHARED-2), which resolves the conversion metric at runtime (cached — `KLAVIYO_CONVERSION_METRIC_ID` if set, else "Placed Order"; ⚠️ **that env var is inert** — Klaviyo's Reporting API accepts a custom conversion metric id and returns base-metric numbers anyway, verified 2026-09-02, so this endpoint **always returns renewal-inclusive revenue**, ~2/3 renewals in the sampled window; never label it acquisition revenue), fetches the campaign/flow lists + `campaign-values-reports` / `flow-values-reports`, and folds the rows per entity via the unit-tested pure shaper `foldKlaviyoValues` (`npm run test:klaviyo-fold`). All via the `klaviyo` singleton's `reportingRequest` passthrough (reuses its auth/revision/backoff).
