...
Thin handler — validates the two required `YYYY-MM-DD` params (Zod; malformed → `400`) and delegates to `getTikTokAdInsights()` in `src/services/admin/tiktok/tiktokAdInsightsQuery.ts` (see [backend.md](./backend.md#tiktok-ad-level-insights-per-ad-spend-breakdown)). Returns `{ success: true, data: { configured, rows, totals, dateRange } }` — `configured` is `false` when the TikTok Marketing-API creds (`TIKTOK_ADVERTISER_ID` + `TIKTOK_MARKETING_ACCESS_TOKEN`) are unset (UI shows "awaiting sync"); `rows` are per-ad, sorted by spend desc, money in dollars, `roas = revenue ÷ spend` (0 when spend 0), plus a summed `totals`. **Revenue is TikTok's own attributed value** (labelled "TikTok rev." in the UI), NOT first-party `PaymentEvent` sales. Populated nightly by the `/api/cron/sync-tiktok-ads` cron (infrastructure domain).
