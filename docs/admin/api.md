...
| `/api/admin/promo-analytics/**` | [promo](../promo/) | Promo-page analytics: summary, channel-detail, page-detail. All gated by `requirePermission("promos.view")`. The three routes share `resolvePromoAnalyticsRange()` from `src/services/promo-analytics/PromoAnalyticsService.ts` for AEST `today \| yesterday \| custom` date resolution, kept in lockstep with the Norm read mirror under `/api/internal/norm/v1/promo-analytics/**`. The summary route's response (2026-07-28) also carries `byPage[].buildDistribution` (full built-prize distribution per page) and `byBuiltPrize` (cross-page, grouped by the combination BUILT rather than the landing page) — see [docs/promo/backend.md](../promo/backend.md#read-side-gap-closure--builddistribution--getaggregatedbybuiltprize-2026-07-28). Neither new field is mirrored to Norm yet (flagged as next-task work). |
