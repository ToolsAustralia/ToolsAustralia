...
| `GET /api/admin/monthly-coupon/trigger-audience` | [rewards-redeemables](../rewards-redeemables/) | **Bonus-code audience forecast (2026-09-01).** Read-only — never mints/issues/redeems. Gated `rewards.view`. Per webhook-minted trigger (`cancel-click`/`checkout-start`/`one-time-purchase`), returns the addressable-population **forecast** (not a holder count — all three campaigns sit at 0 issuances/webhook calls in production), a bounded 25-row sample, and current issued/redeemed counts. Delegates to `BonusCodeAudienceService`. Full contract + the `checkout-start` approximation caveat: [rewards-redeemables/api.md](../rewards-redeemables/api.md#get-apiadminmonthly-coupontrigger-audience--bonus-code-audience-forecast-2026-09-01). |
