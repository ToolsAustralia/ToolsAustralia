...
**`no_held_draft` on-demand re-bill (per-user, 2026-07-20).** When `recoverStrandedPastDueInvoice` hits `no_held_draft` (stranded member, but no held draft to finalize — their next cycle hasn't minted one), the per-user path (`chargeOrRecover` passes `mintCurrentCycleIfNoDraft: true`) falls back to [`mintCurrentCycleInvoice`](../../src/services/subscription/mintCurrentCycleInvoice.ts): unpause + `billing_cycle_anchor:'now'` force-collects the current cycle (auto-charge) and moves the renewal ~1 month out. A run-tagged/`result.recovery.rebill` `InvoiceChargeLog` row records the outcome. **Only the per-user path enables it** (it holds no `RecoveryClaim`, so the mint's own claim is conflict-free); the bulk chunk passes `false` and keeps reporting these as `no_held_draft` skips (it holds a claim + would reset hundreds of anchors unattended). See [CHARGE_PAST_DUE_CUSTOMERS.md](../CHARGE_PAST_DUE_CUSTOMERS.md) and [subscription/backend.md](../subscription/backend.md#mintcurrentcycleinvoiceparams-deps).
