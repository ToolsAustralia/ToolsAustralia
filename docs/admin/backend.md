...
4. Find a held draft. Looks for an existing held draft (Stripe creates one per missed cycle while paused) matching the **expected cycle amount** — which `checkRecoveryEligibility` now derives from the **live Stripe subscription price** (`items.data[0].price.unit_amount`, via `deriveExpectedCycleAmountCents`), falling back to the DB package price only when the live unit price is `null`. This survives a past-due **tier switch** where the DB `packageId` is stale but the held draft is billed at the new price. If none is found, the flow aborts with `reason: "no_held_draft"` and logs a `skipped` row — it does **not** create a new manual invoice because manual invoices have `billing_reason: "manual"` which the webhook dispatch ladder does not handle, breaking the renewal pipeline (no status flip, no entries, no Klaviyo event).
