...
`AdminDashboardStats` also includes `users.activeMembershipsAtEnd?: number` (2026-08-25) — active memberships as of the END of the requested window, the date-scoped counterpart to the standing `users.activeSubscriptions`. It is **absent** when a past window has no `MembershipDailySnapshot` row for its end date (normal for ~3.5h after AEST midnight, before the 17:30 UTC cron). Consumers must DROP the figure when absent rather than substitute `activeSubscriptions` — that would label the live number as that period's. `PeriodComparisonCard` hides its "Total memberships" row entirely unless both windows measured it. See [admin/frontend.md](../admin/frontend.md#membership-rows-added-2026-08-25-renewals-became-past-due-total-memberships).
