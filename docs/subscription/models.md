...
**Writers:** [src/app/api/cron/membership-daily-snapshot/route.ts](../../src/app/api/cron/membership-daily-snapshot/route.ts) (nightly, fires at 18:00 + 19:00 UTC — moved off 14:00/15:00 UTC on 2026-08-24 to clear the renewal-webhook-burst hour; see `docs/infrastructure/gotchas.md`). Write-once upsert: the first fire for a given `{date, packageId}` wins, the second is a no-op fallback for a missed/failed first run.
