...
Drift repair: re-run `npm run backfill:membership-streaks` (recomputes from the ledger; re-runnable, dry-run default via `:dry`). The runner never regresses a live-written reset (skips when the live `streakGeneration` exceeds the computed one).
