...
Read-only cancellation-flow analytics live in the **admin** domain (not subscription): the pure shaper `summarizeCancellationEvents` + DB entry `getCancellationFlowAnalytics` in `src/services/admin/cancellationFlowAnalytics.ts`, the `GET /api/admin/cancellation-flow-analytics` route, and the `CancellationFlowAnalytics` panel (Analytics sidebar tab). The shaper reads `CancellationFlowEvent` (this domain's model) and derives: triggered, per-reason breakdown (count, share, **plus an outcome split** `accepted / cancelled / abandoned` so admins can see which reasons respond to offers vs always churn), the reason→offer→accepted/cancelled/abandoned funnel, save rate, offers-accepted, past-due exclusion from offer-conversion, the 90-day retention split (`retained`/`churned`/`pending`), the same split **per offer** (`retention90ByOffer`), and `otherReasonTexts` (the trimmed `reasonText` of every `reason === "other"` event, sorted newest first, so admins can read what users actually wrote). `abandoned` = `outcome === "in_progress"` AND `startedAt <= now - 1h`; `retention90` only counts matured saves (else `pending`) and is populated by the §6a maturity cron. `retention90ByOffer` (per-`OfferType` retained/churned/pending using the **identical** `savedAt <= now - 90d` matured cutoff as the overall split and the §6a cron — no skew) surfaces in the admin panel so admins can see which offers produce durable saves vs delayed churn. The admin panel uses the shared AEST date filter (`DateRangeToggle` + mobile date-toolbar slot, default `today`); the route accepts `startDate`/`endDate` (yyyy-MM-dd AEST) and the service still receives UTC `Date` bounds. Full contract: [`docs/admin/api.md`](../admin/api.md#cancellation-flow-analytics). Test: `npm run test:cancellation-analytics`.

## Streak-stakes step (spec §7b M3 — 2026-07-15)

One additional screen between reason capture and the offer waterfall: **reason → stakes → offers → confirm**. Implemented as `StepStakes.tsx`, routed by the pure `startPhaseFor(offersShown, pastDue, stakesEnabled)` in `useCancellationFlow.ts` (unit-tested in `test:cancellation-flow-hook`):

- **ALWAYS shown** for non-past-due members while `DASHBOARD_FEATURES.loyaltyStreak` is on (owner decision — visibility never gates on streak depth, only the CONTENT adapts). Feature dark → the pre-streak flow is byte-identical. Past-due keeps its dedicated routing (dunning, not retention).
- **Loss framing (streak ≥ 2):** banked renewals + the next milestone (`nextStreakMilestone`) + the pause-freezes-your-streak reframe. **Forward framing (streak 0/1):** the full ladder; streak 1 gets the "ONE renewal from +100" callout. Copy follows rule 11 (free entries; a streak is *kept*, never bought) and deliberately does NOT advertise the 30-day rejoin grace at the cancel moment.
- **"Continue cancelling" is always visible** — the step informs, never traps. "Keep my streak/membership" mirrors Step4Confirm's keep (closes the modal; no outcome recorded — `saved` stays reserved for accepted offers).
- **Pause card reframe:** `PauseOfferCard` gains a "your N-renewal streak freezes" row when `state.streakMonths ≥ 2`.
- **Instrumentation (server-derived, never client-guessed):** the `start` action stamps `streakMonthsAtStart` on the `CancellationFlowEvent` and returns `streakMonths` in its response; the new `{ action: "stakes", eventId, stakesAction: "kept"|"continued" }` action records the screen exit (fire-and-forget; only while `in_progress`). Save-rate by streak depth is computable from `streakMonthsAtStart × stakesAction × outcome`. A separate A/B experiment was deliberately NOT added: the owner mandated the step always shows, so there is no holdout variant to randomise — the event fields carry the measurement.
- Dev previews: `/dev/cancellation-flow` harness has panels for both framings + the streak-aware pause card.
