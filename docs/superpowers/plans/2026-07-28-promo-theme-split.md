...
4. Measure the p99 of `POST /api/ab-testing/assign` in production. Record it; it is the basis for judging the hold.
5. Activate. Watch: assignment split ≈50/50, the timeout/error branch rate, and LCP on `/promotions/*`.
6. On conclusion, ship the winner as the unconditional default, set the experiment to `ended`, and write a runbook in `docs/ab-testing/` matching `promo-packages-design-runbook.md`.
...
- **Deliberately deferred:** `deliveredAt` on `VariantAssignment` is out of scope unless the error/timeout branch proves to fire often (spec §Out of scope). The fixed 1200 ms guillotine from the first draft is **not** implemented — the hook reveals in light on genuine failure only, and the backstop threshold is set from measured p99 in the runbook.
