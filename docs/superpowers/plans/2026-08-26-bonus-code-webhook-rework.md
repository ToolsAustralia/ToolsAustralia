...
- [ ] **Step 4: The status map (§2).** Implement it exactly, including `409` on a `userId`/`email` mismatch and `403` outside production. The governing rule: non-2xx exists to make Klaviyo retry, so return 2xx wherever a retry cannot change the outcome.
