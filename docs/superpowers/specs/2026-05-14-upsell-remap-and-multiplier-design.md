...
> ✅ Confirmed: the current Apprentice Plus upsell shows 18 entries (`entriesCount: 18` in [src/data/upsellPackages.ts:180](../../../src/data/upsellPackages.ts#L180)). Under the uniform `2×` rule this drops to **6 free entries**, aligning Apprentice with the rest of the one-time upsell ladder.
...
5. **`isMemberOnly` flag on new mini-scoped Additional packs** — ✅ Confirmed `isMemberOnly: true` for the five new `additional-*-pack-mini` records. **However**, mini-scoped purchases must continue to use the `mini-packages` promo type (no multiplier even for subscribers). The flag governs catalog visibility (swap rule) but does NOT route these packs through `getEffectivePromoType`'s subscriber-bonus branch. Implementation must keep mini-scoped packs on the existing mini-draw entry-calculation path; subscribers buying a mini-scoped Additional pack get only the configured `mini-packages` promo multiplier (typically `1×`), not the `membership-packages` multiplier.
6. **Stripe Product ID strategy** — ✅ Confirmed: only membership subscriptions store Stripe Product IDs in code ([membershipPackages.ts:17-23](../../../src/data/membershipPackages.ts#L17-L23)); one-time packs, additional packs, mini packs, and upsells do not. New upsell records follow the existing pattern — no `stripeProductId` field. The new Stripe descriptions from §3.4 are passed at payment-intent / checkout-session creation time, not stored on the upsell record. Implementation plan identifies the exact code path.
