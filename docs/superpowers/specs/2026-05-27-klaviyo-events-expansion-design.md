...
| `giveaways_entered` | number | Count of distinct draws (Major + Mini) the user has at least 1 entry in. Two parallel queries via `Promise.all`: `MajorDraw.countDocuments({ "entries.userId": userId })` (Major Draws store entries as embedded subdocs — indexed at [MajorDraw.ts:269](../../../src/models/MajorDraw.ts#L269)) plus `TicketEntry.distinct("miniDrawId", { userId }).length` (Mini Draws use a flat ticket collection — indexed at [TicketEntry.ts:58](../../../src/models/TicketEntry.ts#L58)). Two queries because Major and Mini draw entries live in different collections; both are indexed and the calls run in parallel. |
