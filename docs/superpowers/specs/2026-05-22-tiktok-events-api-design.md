...
- **Body shape:**
  ```jsonc
  {
    ...
    "data": [{
      ...
      "user": {
        ...
        "phone": "<sha256>",              // hashed: E.164 then SHA-256 hex (NOT `phone_number` — see §2a)
  ...
  ```
- **Hashing:** SHA-256 hex of normalized input for `email` (lowercase+trim), `phone` (E.164 first — **see the correction below re: the field name**), `external_id`, and `first_name`/`last_name`/`zip_code` (lowercase, all whitespace stripped). `city`/`state`/`country` go as lowercase **plaintext**. Never hash `ip`, `user_agent`, `ttclid`, `ttp`. (Same `hashPII` we use for Meta — identical normalization, so browser `ttq.identify` hashes and server hashes match.)
...
- ⚠️ **CORRECTED 2026-07-29 — the field is `phone`. The above was wrong and shipped.** The sources cited were describing the **pixel SDK** (`ttq.identify`) and the **v1.2** Events API, both of which do use `phone_number`; v1.3's `data[].user` uses `phone`. Stape was not an outlier — it was right. v1.3 **silently ignores unknown `user` keys**, so the parameter was dropped on every server-side event with no error and no Diagnostics entry. Detected only in Events Manager → EMQ: `CompleteRegistration` (the only Events-API-**only** event, and one where a valid AU mobile is a *required* signup field) showed **0% phone coverage**, while Purchase/AddPaymentInfo showed 74%/91% — those two also fire from the browser pixel, so their coverage came entirely from `ttq.identify`. Re-verified against Segment's production v1.3 destination (`TTUser.phone`). **Lesson: "matches the pixel" is not evidence about the Events API — the two surfaces have separate vocabularies.**
