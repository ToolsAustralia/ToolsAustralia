...
2. **Ladder (flat across tiers, each fires once per streak generation):** renewal 2 → +100, 4 → +200, 6 → +300, 8 → +400, 10 → +500, 12 → +600 + permanent **Founding member** badge, then **+600 every 12th renewal after** (recurring anniversary).
...
| **P4 — lifecycle hooks** | Klaviyo `streak_months` profile property + events (at-risk, milestone); streak-stakes step **embedded in** CancellationFlowModal per **§7b M3** (ALWAYS shown, content adapts by streak; step order reason → stakes → reframed pause → discount → confirm, cancel path always visible), instrumented via `CancellationFlowEvent` + ab-testing | S | keep cancellation-flow + klaviyo suites green |
