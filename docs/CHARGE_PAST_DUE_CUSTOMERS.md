...
**The up-front decision is necessarily incomplete — the pay branch re-routes post-hoc (2026-07-31).** `decideBulkChargeAction` runs *before* the Stripe call, so it cannot see a PaymentIntent that is live at classify time and dead a fraction of a second later. That is exactly what happens to the **`payment_intent_unexpected_state`** cohort: **245 rows over 28–31 Jul 2026** (1,950 all-time), every one of them an invoice that *was* already recovery-eligible but stayed on the pay branch because a stale `invoice_payment` still read `status: "open"`. `stripe.invoices.pay()` then **cancels that PaymentIntent as part of processing our own request** and rejects the `payment_method` update on it with HTTP 400 / `stripe-should-retry: false` — no charge reaches the issuer. Verified against production: the Stripe request id that returned the 400 is the same request that emitted `payment_intent.canceled`, 17/17 on the 31 Jul cohort. Because our call is what flips the last `open` invoice_payment to `canceled`, the **next** day's run routed the identical cohort to recovery — **5→5, 209→209, 14→14** across three consecutive days.

So: not a card problem, not self-healing (238 of the 245 members were still `past_due`), and **not preventable by inspecting PaymentIntent status up front**. `payOpenInvoiceAsPastDueAdmin` therefore accepts **`deferUnpayableToCaller`**: on this error with `next_payment_attempt == null` it returns `skipReason: "stranded_needs_recovery"` and writes **no** log row, and `chargeWorklistItem` runs `recoverWorklistItem` for that member in the same run. Constraints to preserve:

- **Only the bulk job passes `deferUnpayableToCaller`** — it must write exactly one run-tagged row per worklist item, so the pay attempt and the follow-up recovery can never both log. Every other caller keeps the historical `failed` row.
- **`next_payment_attempt != null` still means stand down** (`awaiting_retry` skip), never recover — Stripe owns the invoice and recovery would void one it is about to retry.
- The change affects **timing only**: the same members were recovered (and re-anchored) 24h later regardless.

Classification lives in the pure `classifyPayFailureRoute` (`src/server/admin/chargePastDuePostPayPolicy.ts`).

Regression-guarded by `npm run test:charge-or-recover-policy` (`decideBulkChargeAction` + `summarizeBulkRecoveryOutcome`), `npm run test:charge-past-due-post-pay` (`classifyPayFailureRoute`), and `npm run test:past-due-history` (decline bucketing).
