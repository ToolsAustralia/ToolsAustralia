...
[`landing-image-resolver.ts`](../../src/utils/promo/landing-image-resolver.ts) builds URLs for promo-landing heroes under `public/images/background/promo/landing/{brand}/`. Filename grammar: `{brand}-{milTB|sidTB|kinTB|gwTB}{-dark}?{-mobile}?{-drawn-tomorrow|-drawn-tonight}?.webp` — note the tier suffix comes **after** `-dark` and `-mobile`. (No landing toolbox ships `-final-hours` art — a `final-hours` request collapses to base; see resolution order below.)

**Current state (2026-09-03 draw-10 urgency export).** Drawn-tier stills ship in **both modes** for the
complete set: **6** brands {dewalt,makita,milwaukee,ryobi,hikoki,stihl} × **4** toolboxes
{milTB,kinTB,sidTB,gwTB} × 2 states × 2 viewports × 2 modes = **192** webp, generated by
[`scripts/convert-draw10-urgency-landing-assets.ts`](../../scripts/convert-draw10-urgency-landing-assets.ts)
(`npm run convert:draw10-urgency-landing`). No combination relies on the light↔dark fallback in
step 2 below — it is a safety net for a future partial drop, not the normal path.

The older [`scripts/convert-drawn-tonight-tomorrow-to-webp.ts`](../../scripts/convert-drawn-tonight-tomorrow-to-webp.ts)
(2026-06/07 drops, light-only, 5 brands × 3 toolboxes = 60 webp, and the `bg-*` stage backgrounds) is
superseded for tier art and kept only as the record of those exports' numbering schemes.
...
**2026-08-28 — draw 10 WITHDREW the tier art; 2026-09-03 restored it.** All 160 draw-9 tier files
baked `& $5K CASH` into the headline, and draw 10 removed that bonus — so they advertised a prize we
no longer give, on the highest-intent surface we have, inside a countdown window nobody routinely
looks at. They were deleted, `URGENCIES` in
[`scripts/check-landing-hero-assets.mjs`](../../scripts/check-landing-hero-assets.mjs) was cut to
`[null]`, and both tiers fell through to the base hero (which the resolver already does for
`final-hours`). The replacement export landed 2026-09-03 and both halves were restored together.

Three things worth knowing about the draw-10 urgency drop:

- **The tier axis is a filesystem accident, not a naming convention.** Each `toolbox × toolset ×
  viewport` ships FOUR files — `GD - 1.png`, `GD - 1 (2).png`, `GD - 2.png`, `GD - 2 (2).png`. The
  digit is light/dark (still measured by luminance, never parsed); the ` (2)` is the OS duplicate
  marker from extracting the two tier folders over each other, and it maps plain → `drawn-tomorrow`,
  `(2)` → `drawn-tonight`. **That is an observation about one export, not a rule** — a re-download in
  the other order flips it, and no test can catch a swap. Run the converter with `--sheets` and read
  the four contact sheets before trusting a new drop.
- **Copy was verified against the live base before installing** — the specific failure that killed the
  draw-9 set. 24/24 mobile and 40/48 desktop `combo × mode` groups carry a headline region
  pixel-identical to the base hero already serving; the 8 that differ were read by eye. None says
  `& $5K CASH`; all say "OR TAKE HOME $10,000". The tier art carries the same headline as its own
  base, so a countdown rollover never changes the offer — only the badge.
- **Half the drop is a fresh render, not "base plus a badge".** On 27/48 desktop and 12/48 mobile
  groups the product arrangement, model pose and layout all changed (13–34% of pixels); the rest
  differ 2–5% (badge added to the same photo). Cosmetic, but it means a countdown rollover visibly
  re-shoots the hero on those pages — and it is why light/dark cannot be inferred by diffing against
  the base.
...
On the `drawn-tomorrow` / `drawn-tonight` calendar-day tier it swaps to the animated badge clip
`{brand}-{toolbox}{-dark}?{-mobile}?-{drawn-tomorrow|drawn-tonight}.{webm,mp4}`. As of the 2026-09-03
draw-10 urgency export that is **192 drawn clips** (6 brands × 4 toolboxes × 2 viewports × 2 modes ×
2 tiers), each shipping a WebM + MP4, generated by
[`scripts/convert-draw10-urgency-landing-assets.ts`](../../scripts/convert-draw10-urgency-landing-assets.ts)
via ffmpeg — MP4 is a stream copy (the export is already h264 at target dimensions), WebM is VP9
`-crf 34`. `final-hours` / `null` keep the base hero clip.
