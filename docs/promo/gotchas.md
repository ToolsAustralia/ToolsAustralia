...
The promo hero / giveaway countdown / entry CTAs don't own a modal — `useMajorDrawEntryCta.openEntryFlow({ openLocalModal: false })` dispatches a `window` `"openMembershipModal"` CustomEvent (the major-draw purchase gate is already applied; **members** with additional-package access are routed to the special-packages modal instead, so the event only fires for non-members). Whatever **package section** renders on the page is expected to listen for that event and open its `MembershipModal`. The A/B **treatment** (`PromoMembershipDesign`) originally didn't, so the hero "Enter Now" silently no-op'd for non-members on the treatment arm. Fixed with a shared [`useOpenMembershipModalListener`](../../src/hooks/useOpenMembershipModalListener.ts) hook (gating built in) used by **both** the control (`MembershipSection`) and the treatment — one line, impossible to half-implement, so a future section variant can't forget the plumbing. The treatment's multiplier header is also now gated on `hasMultiplierBanner(...)` (text fallback when no banner art exists), matching the control.
