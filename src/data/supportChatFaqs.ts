...
export function getSupportChatFaqEntries(): FaqEntry[] {
  ...
  return [
    ...
    // -- BONUS CODES (per-customer, personal deadline) -----------------------
    {
      ...
      answer:
        "Your bonus code has its own deadline, and it belongs to you alone -- the clock starts the moment the code is created for you, not on a shared cut-off date, so two members who get the same code on different days will have different deadlines. It then runs for a fixed 72 hours from that moment, which means it can run out at any time of day: it is not an end-of-day cut-off and it is not tied to a whole calendar date. The exact date and time is printed in the email that carries your code, and that is the one we enforce -- so if you are not sure, check that email. Redeem it at checkout before then and the free entries it includes are added to your next Major Draw. If the window has already passed, the code no longer works and the free entries it includes are not added -- and when you are signed in, the message at checkout names the exact date and time it ran out.",
      category: "REWARDS",
    },
    {
      id: "87",
