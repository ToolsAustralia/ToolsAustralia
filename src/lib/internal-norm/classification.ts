...
export const NORM_ENDPOINTS = {
  ...
  "users.get": {
    ...
    path: "/v1/users/:id",
    method: "GET",
    summary: "Single-user detail with statistics counts + membership signals: subscription (incl. cancelledAt, nextRenewalEntries — what the next renewal grants) and partnerAccessRing (partner-catalogue access % + state). PII-safe: firstName + state only; email/lastName/mobile/address/savedPaymentMethods/orders array stripped",
