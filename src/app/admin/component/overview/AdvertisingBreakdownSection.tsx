...
export default function AdvertisingBreakdownSection({
  ...
  const SortHeader = ({ column, children }: { column: SortKey; children: React.ReactNode }) => {
    ...
    return (
      <th
        className="text-center px-1 py-1.5 sm:px-2 sm:py-2.5 font-semibold text-gray-700"
  ...
  return (
      ...
      headerTrailing={
        <div className="flex items-center gap-1.5 sm:gap-2">
          {isFetching && !isLoading && (
            <span className="text-[10px] sm:text-xs text-gray-500 max-w-[5rem] sm:max-w-none truncate sm:whitespace-nowrap">
              Refreshing…
            </span>
          )}
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 sm:gap-2 px-2.5 py-1 sm:px-4 sm:py-2 rounded-lg bg-slate-900 text-white text-[11px] sm:text-sm font-medium hover:bg-slate-800 disabled:opacity-50 shrink-0"
          >
    ...
    >
      {syncMutation.isError && (
        <div className="text-xs sm:text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-2.5 py-1.5 sm:px-3 sm:py-2 leading-snug mb-3">
          {syncMutation.error instanceof Error ? syncMutation.error.message : "Sync failed"}
        </div>
      )}

      {syncMutation.isSuccess && !syncMutation.isPending && (
        <div className="text-xs sm:text-sm text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-2.5 py-1.5 sm:px-3 sm:py-2 leading-snug mb-3">
      ...
      {!isLoading && promotionMetrics.length > 0 && (
        <div className="overflow-x-auto -mx-3 px-3 sm:-mx-4 sm:px-4 lg:-mx-5 lg:px-5 [scrollbar-gutter:stable]">
          {/* min-width + horizontal scroll on narrow viewports; equal column share avoids huge gaps */}
          <div className="bg-white rounded-md sm:rounded-lg border border-gray-200 overflow-hidden min-w-[540px] sm:min-w-0 sm:w-full">
            <table className="w-full table-fixed text-[10px] sm:text-xs md:text-sm border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {sortedMetrics.map((metric) => (
                  <tr key={metric.slug} className="hover:bg-gray-50">
                    ...
                    <td className="px-1 py-1.5 sm:px-2 sm:py-2.5 text-center text-gray-700 tabular-nums font-semibold whitespace-nowrap align-middle">
                      {metric.roas.toFixed(2)}x
                    </td>
                    <td className="px-1 py-1.5 sm:px-2 sm:py-2.5 text-center text-gray-700 tabular-nums whitespace-nowrap align-middle">
