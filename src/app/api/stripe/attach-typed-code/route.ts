...
import {
  ...
  type TypedCodeCheckoutTarget,
} from "@/utils/payment/attach-typed-code";

/**
 * Stamps the customer's typed discount code onto the checkout object they are
 * about to pay — called by `MembershipModal` at the PURCHASE click, after the
 * code is final and BEFORE `confirmPayment`.
 *
 * ALL THREE CODE TYPES. The body carries the RAW string the customer typed and
 * says nothing about its kind; the service classifies it (referral -> promo ->
 * campaign, exactly as `/api/codes/validate` does) against an identity resolved
 * from the Stripe object's own server-written metadata, and writes it into the
 * matching key. That is what makes it safe to widen — the browser gains no new
 * trust, because it never claims a type. Referral and promo-link codes ride only
 * in a create-subscription body otherwise, so on the pre-warmed doors where that
 * create call is skipped this seam is the ONLY thing that delivers them.
 *
 * The decision lives in `@/utils/payment/attach-typed-code`; this handler
...
const attachTypedCodeSchema = z
  .object({
    /**
     * The RAW string the customer typed, or `null` to CLEAR a stale stamp.
     * Never a classification — the server decides which of the three it is.
     */
...
const attachTypedCodeRateLimiter = createRateLimiter("attach-typed-code", {
  windowMs: 60 * 1000,
  maxRequests: 60,
});
...
async function resolveSessionUserId(): Promise<string | undefined> {
  try {
    ...
    console.error("[attach-typed-code] session resolution failed — continuing on object metadata", error);
    return undefined;
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResult = attachTypedCodeRateLimiter.check(identifier);
    ...
    const parsed = attachTypedCodeSchema.parse(await request.json());

    const target: TypedCodeCheckoutTarget =
    ...
    const result = await attachTypedCodeToCheckout({
      target,
      code: parsed.code,
    ...
    if (result.ok) {
      // `slot` is what lets the success screen tell the truth about WHICH kind of
      // code landed — the caller may only claim a code applied once it actually
      // reached the server.
      return NextResponse.json({ success: true, code: result.code, slot: result.slot });
    }
    ...
    console.error("[attach-typed-code] request failed", error);
