...
function PayForm({ totalLabel, onPaid }: PayFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: stripeError, paymentIntent } = await stripe.confirmPayment({
      elements,
      confirmParams: { return_url: getReturnUrlForPaymentTypeClient("shop") },
...
export default function ShopCheckoutPaymentElement({
  ...
  return (
    <Elements stripe={getStripePromise()} options={options}>
      <PayForm totalLabel={totalLabel} onPaid={onPaid} />
    </Elements>
  );
}
