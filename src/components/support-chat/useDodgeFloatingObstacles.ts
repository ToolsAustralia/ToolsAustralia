 ...
 *
 * Keeps the floating support-chat launcher from overlapping other bottom-anchored
...
// Launcher geometry — must match SupportChatWidget's bubble classes (w-14 h-14, bottom-5, {left,right}-5).
const LAUNCHER_PX = 56; // w-14 / h-14 = 3.5rem
const BASE_BOTTOM = 20; // bottom-5 = 1.25rem
const BASE_SIDE = 20; // left-5 / right-5 = 1.25rem
...
export function useDodgeFloatingObstacles(
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      // Launcher's DEFAULT rect (before any dodge), in viewport coords.
      const x0 = side === "right" ? vw - BASE_SIDE - LAUNCHER_PX : BASE_SIDE;
      const x1 = x0 + LAUNCHER_PX;
      const y0 = vh - BASE_BOTTOM - LAUNCHER_PX;
      const y1 = vh - BASE_BOTTOM;
