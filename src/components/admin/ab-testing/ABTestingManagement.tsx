...
export default function ABTestingManagement() {
  ...
  return (
    <div className="w-full space-y-3 sm:space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 lg:gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="min-w-[120px] sm:min-w-[150px] rounded-lg border-2 border-gray-300 bg-white px-3 py-1.5 sm:py-2 text-xs sm:text-sm focus:ring-2 focus:ring-red-500/50 focus:border-red-500 shadow-sm hover:shadow-md transition-all duration-200"
            >
      ...
      {/* Experiments Table */}
      <div className="bg-white rounded-xl shadow-lg border-2 border-gray-200/50 overflow-hidden">
        {isLoading ? (
          <div className="p-8 sm:p-12 text-center">
            <RefreshCw className="mx-auto h-6 w-6 sm:h-8 sm:w-8 animate-spin text-gray-400" />
            <p className="mt-3 sm:mt-4 text-sm sm:text-base text-gray-500">Loading experiments...</p>
          </div>
        ) : error ? (
          <div className="p-8 sm:p-12 text-center">
            <p className="text-sm sm:text-base text-red-600">Error loading experiments</p>
            <button
              onClick={() => refetch()}
          ...
          <div className="p-8 sm:p-12 text-center">
            <Target className="mx-auto h-10 w-10 sm:h-12 sm:w-12 text-gray-400" />
            <p className="mt-3 sm:mt-4 text-sm sm:text-base text-gray-500">No experiments found</p>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b-2 border-gray-200">
                <tr>
                  ...
                  <th className="px-3 sm:px-4 lg:px-6 py-2 sm:py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">
                    Actions
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 bg-white">
                {experiments.map((experiment) => (
                  <tr key={experiment._id} className="hover:bg-gray-50 transition-colors">
                    <td className="whitespace-nowrap px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
                      <div className="font-medium text-sm sm:text-base text-gray-900">{experiment.name}</div>
                    </td>
                    <td className="whitespace-nowrap px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
                      <span
                    ...
                    <td className="whitespace-nowrap px-3 sm:px-4 lg:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-500">
                    ...
                    <td className="whitespace-nowrap px-3 sm:px-4 lg:px-6 py-3 sm:py-4 text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-1 sm:gap-2">
                        <button
                          ...
                          className="rounded p-1.5 text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-colors"
                          title="View Details"
                        >
                        ...
                        {experiment.status === "active" && (
                          <button
                            ...
                            className="rounded p-1.5 text-yellow-600 hover:bg-yellow-50 hover:text-yellow-700 disabled:opacity-50 transition-colors"
                            title="Pause"
                          >
                        ...
                        {(experiment.status === "active" || experiment.status === "paused") && (
                          <button
                            ...
                            className="rounded p-1.5 text-red-600 hover:bg-red-50 hover:text-red-700 disabled:opacity-50 transition-colors"
                            title="End"
                          >
