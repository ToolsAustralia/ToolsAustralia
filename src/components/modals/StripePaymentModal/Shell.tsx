...
interface ShellProps {
  isOpen: boolean;
  onClose: () => void;
  /** Drives the hero color theme. */
  tone?: StripePaymentTone;
  /** Eyebrow content — small uppercase label above the headline. */
  eyebrow: ReactNode;
  /**
   * Headline content. Wrap accent words in `<span data-rf-accent>…</span>` to
   * apply the tone color automatically.
   */
  title: ReactNode;
...
const Shell: React.FC<ShellProps> = ({
  ...
  /** Entry-animation gate — 10 ms setTimeout matches RenewalFailedModal. */
   ...
   * Cleanup ALWAYS resets both overflow properties — we do NOT capture the previous value.
   */
  useEffect(() => {
    if (!isOpen) return;
    ...
    >
      ...
      <div
        className={cn(
          ...
          "w-full max-w-[520px] font-sans text-neutral-950 antialiased",
        ...
        >
          ...
          >
            ...
            <div className="relative z-[2]">
              ...
              {/* Lock icon — Stripe-secured feel */}
              <div className="flex justify-center mb-2 max-xs:mb-1.5">
                <span
                  className="inline-flex items-center justify-center"
                  style={{ color: "var(--tone-accent)" }}
                >
                  <Lock size={32} strokeWidth={2} className="max-xs:w-7 max-xs:h-7" />
                </span>
              </div>
