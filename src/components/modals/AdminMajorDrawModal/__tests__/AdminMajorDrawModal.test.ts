/**
 * Smoke test for AdminMajorDrawModal. Renders meaningful prop combos via
...
import AdminMajorDrawModal from "../index";
...
interface Combo {
  name: string;
  props: React.ComponentProps<typeof AdminMajorDrawModal>;
}

const combos: Combo[] = [
  {
    name: "open — initial state",
    props: { isOpen: true, onClose: noop },
  },
  {
    name: "open with onSuccess callback",
    props: { isOpen: true, onClose: noop, onSuccess: noop },
  },
  {
  ...
  {
    name: "closed with onSuccess callback",
    props: { isOpen: false, onClose: noop, onSuccess: noop },
  },
];

console.log("\nAdminMajorDrawModal smoke test");
...
for (const combo of combos) {
  test(combo.name, () => {
    ...
    const modalEl = React.createElement(AdminMajorDrawModal, combo.props);
