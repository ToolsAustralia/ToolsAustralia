...
const RenewalShell: React.FC<{
  ...
  /**
   * data-testid for the modal root. Defaults to "renewal-failed-modal" so
   * Playwright specs (e.g. e2e/membership/renewal-failed.spec.ts) can target
   * the wrapper without needing each call site to pass it explicitly.
   */
  testId?: string;
}> = ({ isOpen, onClose, tone = "danger", eyebrow, title, sub, children, closeOnBackdrop = false, testId = "renewal-failed-modal" }) => {
  useEffect(() => {
    if (!isOpen) return;
  ...
  return (
    <div data-testid={testId} className="rf-root fixed inset-0 z-[80] flex items-center justify-center p-2 sm:p-4 overflow-hidden">
      <div
