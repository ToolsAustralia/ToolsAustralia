...
/**
 * Inline Stripe SetupIntent card form, shown when the user has no saved payment
 * methods. Wraps the form in <Elements> using a module-scope stripePromise
 * passed from the orchestrator (Stripe singleton lives at module scope, never
 * re-instantiated per render).
 */
const PaymentSection: React.FC<PaymentSectionProps> = ({
  ...
  if (!needsInlineCardSetup || !hasSelectedPackage) return null;
  return (
      ...
      {setupIntentSecret && !loadingSetupIntent && (
        ...
        >
          <StripeInlineCardSetupForm
            ...
            submitLabel={`Save card & buy — $${selectedPackagePrice ?? 0}`}
          />
        </Elements>
      )}
    </div>
