...
const Hero: React.FC<HeroProps> = ({ entriesCopy, accumulatedEntries }) => {
  return (
    <UpsellHero
      tone="neutral"
      titleId="cm-headline"
      className={cn(styles.heroBg, styles.heroStripeOverlay)}
      eyebrow={
        <>
      ...
      title={
        <>
          <span className="block font-sans font-extrabold text-[11px] tracking-[0.14em] text-white/55 uppercase mb-1 max-xs:text-2xs max-xs:mb-0.5">
            You&apos;re already in.
          </span>
      ...
      infographic={
        <>
          {/* Prize banner */}
          <div
            className="rounded-xl overflow-hidden border border-white/10 leading-none max-xs:rounded-[10px]"
            style={{
