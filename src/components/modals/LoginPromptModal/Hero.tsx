...
interface HeroProps {
  /** ID for the headline element — wired into Shell's aria-labelledby. */
  titleId: string;
  /** Eyebrow pill copy (e.g. "Login required"). */
  eyebrow: string;
  /** Big uppercase headline copy. */
  title: string;
  /** Sub-copy beneath the headline. */
  sub: string;
  /** Lucide icon component shown inside the glowing tile next to the headline. */
  icon: ComponentType<{ className?: string; strokeWidth?: number }>;
}

/** LoginPromptModal hero — dark gradient backdrop with red glow, eyebrow pill,
 * glowing icon tile + 2-row text column. Mirrors the design language used by
 * PackageDetailModal/Hero, UpgradeConfirmModal/Hero, etc. */
