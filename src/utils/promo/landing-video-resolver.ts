 ...
 *
 * On the `drawn-tomorrow` / `drawn-tonight` calendar-day tier, the animated badge clips
 * ship for every brand × toolbox (`{brand}-{toolbox}{-mobile}-drawn-{state}.{webm,mp4}`), so
 * the urgency suffix swaps the base hero clip for its drawn variant. `final-hours` has no clip
 * and falls back to the base hero.
 */
export function getLandingHeroVideoPaths(
  ...
  return {
    desktop: { webm: `${base}${drawnSuffix}.webm`, mp4: `${base}${drawnSuffix}.mp4` },
    mobile: { webm: `${base}-mobile${drawnSuffix}.webm`, mp4: `${base}-mobile${drawnSuffix}.mp4` },
  };
}
