...
- **`MonthlyEntryCampaign` codes** — admin-issued bonus-entry codes redeemed via `RedeemableIssuance` (see §8). One-per-user, status flips to `redeemed` on consumption. A campaign with `validForHours` set is a **trigger campaign** (see §7d).
  - **A claim is only spent once the entries are actually in a draw (fixed 2026-08-27).** `RedemptionService.redeem()` burns the issuance atomically (that write is the only guard against two concurrent claims double-granting), then grants — and if the grant does not land (a freeze/gap window with no draw to receive it) it **reverses the whole claim**: issuance back to `active`, the `redeemedEverAt` this call stamped removed, wallet counter and history row undone. The customer keeps their one-per-lifetime grant and is told plainly that nothing landed, instead of being told "N free entries added" while receiving none. Same rule as `/api/cancellation-upsell/redeem` — a caller granting an entitlement must treat a failed grant as *not delivered*. See [rewards-redeemables/gotchas.md](docs/rewards-redeemables/gotchas.md).
  - **…but a claim is only REVERSED when the failure is provably a non-delivery (hardened 2026-08-27).** The reversal above was applied on a failed grant AND on a thrown one, as if they were the same fact. They are not: a throw can come from the draw document's own `save()`, i.e. *after* the entries were added to it, so a lost acknowledgement on a write the server actually applied would leave the entries in the live draw **and** hand the code back — the customer's next claim then lands the same 200 a second time against one 200-entry code. `DrawGrantService.grantMonthlyCouponEntries` now answers with three states instead of a boolean (`landed` / `not_written` / `unconfirmed`), verifying against a pre-mutation baseline before it decides, and only `not_written` is reversed. An `unconfirmed` claim reverses **nothing**, answers the new `grant_unresolved` reason (HTTP 500 — not the retry-inviting 503) and is logged for a human. The deliberate trade: a stuck claim an admin can fix, never a second grant into a live draw that nobody can withdraw. The same three-state rule governs the streak auto-grant and `/api/cancellation-upsell/redeem`. The claim's wallet write is now guarded too — it used to be able to throw straight out of `redeem()`, leaving the issuance `redeemed` with `redeemedEverAt` stamped and nothing granted.
...
**Blocker cleared 2026-08-27 — a shared code could only ever reach ONE customer.** All three
codes are set up as a shared, everyone-types-the-same-thing campaign. For that kind of campaign the
system writes a customer record with no personal code on it — and a database rule, by accident,
allowed only ONE such record per campaign. The first customer enrolled worked; the second and
everybody after hit a database rejection that the code mistook for “this person already has one”
and quietly did nothing, so they received no email and, if the marketing flow sent the discount mail
as its own step, could see APPLIED at checkout, pay, and get nothing. In practice each of BACKIN200,
LOCKIN100 and EXTRA100 was a one-customer code. Fixed in three parts: the database rule now skips
code-less records entirely; a duplicate-record error is classified by which rule actually fired
rather than guessed at; and “they already have one” is only ever reported when a record genuinely
exists. **A migration must be run against production before the sequences go out** —
`npm run migrate:issuance-partial-code-index:prod` (dry-run first with `:prod:dry`); a code change
alone does not re-option an index that already exists in a live database. Add it to the launch
runbook ahead of step 2 (creating the campaigns).

**Also closed 2026-08-27 — a slow checkout could lose an applied code.** The browser gives up on the
request that attaches a code to a payment after 15 seconds and takes the payment anyway (correctly —
a bonus lookup must never block a sale). That happened for real in testing: the server finished the
work at 14.9 seconds, the browser had already stopped listening, the card was charged, and the
entries did not land. The server now writes down that the customer applied the code **before** it
talks to Stripe, and the payment webhook finishes the job from that record when the payment arrives
without one. The customer keeps their code either way; what this recovers is the entries on the
purchase they just made. Removing a code still removes it, and the record expires after 30 minutes
so it can never attach itself to a later purchase or a renewal.
...
- **An upgrade no longer destroys the anchor (2026-08-24).** A tier upgrade is deliberately pay-first (`billing_cycle_anchor: "now"`, §10c), which Stripe **rejects outright** while an anchor `trial_end` is still pending — *"Trial end (…) cannot be after billing_cycle_anchor (…)"*. Every upgrade attempt by an anchored member therefore failed with a Stripe 400, permanently, for the whole 25/26/27-joiner cohort. The upgrade route now **ends the trial in the same call** (so the charge can land), then **re-applies the member's own pending `trial_end`** for the next cycle — pay-first semantics and the anchor both survive. Only if that anchor has already lapsed does it fall back to the shared next-24th helper (`getNextAnchorTimestamp`, the same one the join rule and the migration use). Members not on an anchor trial are unaffected.
- **Double-charge floor on the re-applied anchor (14 days).** `trial_end` is a **billing boundary** — Stripe charges the full amount when it arrives — so re-applying an anchor only days after an upgrade would bill the member twice within days, with `proration_behavior: "none"` meaning no credit for the overlap. The route therefore keeps the member's anchor **day** but advances to the next **occurrence** whenever the nearest one is under 14 days (half a cycle) away, via the same `getReanchorTrialEndTimestamp` the past-due reanchor uses. A member who upgrades within a fortnight of their anchor renews the *following* month, so a shifted renewal is always a full month ahead rather than days away. It is a **floor, not a full-month guarantee**: an upgrade 20 days before the anchor keeps that date and buys 20 days, since there is no pro-rata credit in either direction. Erring generous is deliberate: the failure on the other side is double-charging a paying member. See [upgrade-subscription-payment/route.ts](src/app/api/stripe/upgrade-subscription-payment/route.ts) and `docs/PAST_DUE_REANCHOR.md`.
- Source of truth: `docs/BILLING_ANCHOR_24.md` (join-anchor rule), `docs/PAST_DUE_REANCHOR.md` (past-due reanchor + the upgrade re-apply).
- **$0 trial-invoice double-grant guard.** Any mutation that sets `trial_end` on an *existing* subscription — the 25/26/27→24 join-anchoring above, anchor-billing migrations, past-due reanchor, and (since 2026-08-24) the **upgrade anchor re-apply** — makes Stripe auto-spawn a separate **$0 invoice** with `billing_reason: subscription_update` and mark it paid — firing a second `invoice.payment_succeeded`. The webhook detects it via [`isZeroAmountTrialUpdateInvoice`](src/utils/billing/trial-invoice.ts) (`subscription_update` + `total === 0` + `amount_paid === 0`) and **skips it entirely** ([stripe-webhook-handlers](src/services/stripe-webhook-handlers/index.ts)), so it does **not** grant a second set of renewal entries or log a spurious "Subscribed to X" admin-activity row. The match is deliberately narrow — a 100%-off renewal is `subscription_cycle` and a real upgrade proration has `total > 0`, so both still grant. Idempotency-by-id does **not** protect you here (the spawned invoice has its own id). Shipped 2026-06-02; regression-tested via `npm run test:zero-trial-guard`.
...
- **3DS/SCA on Pay-Now was returning a 500 (fixed 2026-09-03).** When a member's bank demanded a 3D Secure challenge, `stripe.invoices.pay()` threw `invoice_payment_intent_requires_action` and `pay-failed-invoice` answered **HTTP 500 "Failed to initialize payment"** instead of handing back the PaymentIntent for the browser to confirm — so those members **could not pay their renewal at all**. It was every production 500 in the week to 2026-09-03; one member retried 7× in 16 minutes on 2026-09-02 and was blocked each time. The endpoint's recovery step was gated on the error *message* mentioning a missing default payment method, which a 3DS error never says. It now recovers the PaymentIntent from the invoice whenever the failure is not terminal, so 3DS returns the normal `requiresPaymentConfirmation` response the client already handles. See `docs/payment/gotchas.md`.
...
- **Cards Stripe has blocked for excessive retries now sit out 3 days (2026-08-18).** Stripe support confirmed that a block carrying `previously_declined_do_not_retry` comes from **Adaptive Acceptance**, not Radar — the card-fingerprint allowlist **cannot** override it, and no account setting disables it. It is triggered by our own retry velocity ("too many payment attempts… in a short time window"), and Stripe's guidance is to leave **2–3 days between retries of the same transaction**. Measured on 17 Aug: 69 of 72 blocks that day were this reason, and 70 of the 72 were on cards already on the live allowlist; across all history **835 of 1,024 recorded blocks (82%)** carry it. So the daily re-attempt on those cards collected nothing *and* sustained the block. The bulk run now skips them for 3 days, bucketed as **"Retry in 3 days"**. **Revenue impact is protective, not a reduction**: the block decays on its own (118 of 701 blocked cards, 17%, later succeeded — some 3+ months on), so the cohort is **slowed, not dropped**. The cooldown is keyed to the **card**, so a member who adds a new card is charged immediately, and Radar-type blocks (which allowlisting does fix) are untouched. ⚠️ Stripe also flagged **batch** velocity — spreading the run rather than firing the whole worklist at once — which is **not yet implemented**.
...
- User pays the **full new-tier price immediately**; renewal date resets to today — **except for an anchor-24 member, whose renewal day is preserved** (§9b). Those members sit on a Stripe `trial_end`; the route ends that trial to take the charge, then re-applies it for the next cycle, so the member is charged today and still renews on their anchor day. The Stripe subscription is left `trialing`, which the UI renders as **"Active"** — the same cosmetic artifact as every other anchored member (§9b, §10a).
- **An upgrade never lands two full charges in the same fortnight.** Because the re-applied anchor is a billing boundary, an upgrade close to the anchor would otherwise charge full price twice in days. The renewal is pushed one cycle when the anchor is under 14 days out — see §9b.
- **Anchored upgrades used to be impossible.** Before 2026-08-24 the pay-first `billing_cycle_anchor: "now"` collided with the pending `trial_end` and Stripe returned a 400 on **every** attempt — a permanent, recurring revenue leak for the 25/26/27-joiner cohort, not an intermittent failure. Verified end-to-end against live Stripe test mode by `npm run stripe:probe-upgrade-anchor` (10/10), which reproduces the original rejection as a control and confirms the fixed sequence produces exactly one full-price charge.
...
- **Meta / Facebook**: Pixel (client) + CAPI (server-side, `src/lib/facebook.ts`, test `npm run test:facebook-capi`). Identity enrichment (2026-05-25) threads hashed user data (email/name/phone, fbc/fbp) through the CAPI mirror for AddPaymentInfo / InitiateCheckout / CompleteRegistration to lift Event Match Quality. _2026-08-17: added the **`ge` (gender)** Advanced-Matching parameter — previously documented as deliberately not collected. It is sourced from a new **optional** `User.gender` field, hashed as Meta's single lowercase letter (`m`/`f`), and threaded through every Meta hash site (browser AM, CAPI provider, legacy `prepareUserData`, registration) so browser and server produce identical hashes. Members who never answered send **no `ge` at all**. Because the field is optional and new, coverage starts at 0% and grows only as members fill it in — no immediate match-quality lift should be expected._
...
- **Placed Order** events are tagged with `is_renewal`, so renewal revenue is distinguishable from first-purchase revenue in Klaviyo flows/segments. Stripe-originated orders also carry the raw `billing_reason`; merchandise orders carry `is_renewal: false` and no `billing_reason`. **Caveat (verified 2026-09-02):** the renewals-excluded "Marketing Revenue" split is readable in the **Klaviyo UI only** — Klaviyo's Reporting API accepts a custom conversion metric id and then returns base (renewal-inclusive) numbers regardless, so the admin Klaviyo tab and any API pull show revenue that **includes** automated renewals (roughly two-thirds of the attributed total in the sampled window).
