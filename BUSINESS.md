...
| **Resumed by**           | successful payment                               | Stripe auto-resume at `resumes_at` (30 days); the `cancellation-retention-resume` cron defensively resumes if Stripe hasn't after the window, then clears the retention metadata. No user-triggered early return exists. |
...
| **Membership Streak (loyalty streak)**  | Backend P1 live (dark); UI built, hidden     | Consecutive paid renewals earn escalating **free entries** auto-granted into the Major Draw: rungs at renewals 2/4/6/8/10/12 → +100…+600 + permanent Founding-member badge, then +600 every anniversary — flat across tiers, join = month 0. **P1 shipped 2026-07-07**: durable `subscription.streakMonths`/`streakGeneration` counter written replay-proof by the Stripe webhook + re-runnable backfill (`backfill:membership-streaks`). Grants (P2) and the dashboard card (P3, `DASHBOARD_FEATURES.loyaltyStreak`) pending. Spec: [docs/superpowers/specs/2026-07-07-membership-streak-design.md](docs/superpowers/specs/2026-07-07-membership-streak-design.md). |
