...
| **Membership Streak (loyalty streak)**  | Backend P1+P2 built (dark); UI built, hidden | Consecutive paid renewals earn escalating **free entries** auto-granted into the Major Draw: rungs at renewals 2/4/6/8/10/12 → +100…+600 + permanent Founding-member badge, and the **full ladder repeats every streak year** (month 14 ≡ month 2 → +100 again … month 24 ≡ month 12 → +600) — flat across tiers, join = month 0. **P1 (counter + backfill) and P2 (milestone engine: `streak-months` type, autoGrant straight-to-draw under a first-class `streak` entry bucket, generation-scoped re-earn, `seed:streak-rewards` ladder — seeded `isActive:false`) built 2026-07-07, both dark.** Goes customer-visible at P3 (`DASHBOARD_FEATURES.loyaltyStreak` flip + rung activation). Spec: [docs/superpowers/specs/2026-07-07-membership-streak-design.md](docs/superpowers/specs/2026-07-07-membership-streak-design.md). |
